...
void ImGui::TextEx(const char* text, const char* text_end,
    ...
    if (text_end - text <= 2000 || wrap_enabled) {
        ...
        // height, which is unlikely to matter because we are likely the biggest
        // and only item on the line.
        // - We use memchr(), pay attention that well optimized versions of
        // those str/mem functions are much faster than a casually written loop.
        ...
        if (!g.LogEnabled) {
            ...
            if (lines_skippable > 0) {
                ...
                while (line < text_end && lines_skipped < lines_skippable) {
                    ...
                    if (!line_end)
                        line_end = text_end;
                    if ((flags & ImGuiTextFlags_NoWidthForLargeClippedText) ==
                        0)
                        text_size.x =
                            ImMax(text_size.x, CalcTextSize(line, line_end).x);
                    line = line_end + 1;
                    lines_skipped++;
                }
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args) {
    ...
    const ImVec2 value_size =
        CalcTextSize(value_text_begin, value_text_end, false);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);

    const ImVec2 pos = window->DC.CursorPos;
    const ImRect value_bb(
        pos, pos + ImVec2(w, value_size.y + style.FramePadding.y * 2));
    const ImRect total_bb(
        pos, pos + ImVec2(w + (label_size.x > 0.0f
                                   ? style.ItemInnerSpacing.x + label_size.x
                                   : 0.0f),
                          ImMax(value_size.y, label_size.y) +
...
// FIXME: For refactor we could output flags, incl mouse hovered vs nav keyboard
// vs nav triggered etc. And better standardize how widgets use
// 'GetColor32((held && hovered) ? ... : hovered ? ...)' vs 'GetColor32(held ?
// ... : hovered ? ...);' For mouse feedback we typically prefer the 'held &&
// hovered' test, but for nav feedback not always. Outputting hovered=true on
// Activation may be misleading.
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered,
    ...
    if (g.DragDropActive && (flags & ImGuiButtonFlags_PressedOnDragDropHold) &&
        ...
        if (IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem)) {
            hovered = true;
            SetHoveredID(id);
            if (g.HoveredIdTimer - g.IO.DeltaTime <=
    ...
    // Mouse handling
    const ImGuiID test_owner_id =
        (flags & ImGuiButtonFlags_NoTestKeyOwner) ? ImGuiKeyOwner_Any : id;
    if (hovered) {
        IM_ASSERT(id != 0); // Lazily check inside rare path.

        // Poll mouse buttons
        // - 'mouse_button_clicked' is generally carried into
        // ActiveIdMouseButton when setting ActiveId.
        // - Technically we only need some values in one code path, but since
        // this is gated by hovered test this is fine.
        ...
        for (int button = 0; button < 3; button++)
            ...
            {
                if (IsMouseClicked(button, ImGuiInputFlags_None,
                                   test_owner_id) &&
            ...
            if (g.ActiveId == id && (item_flags & ImGuiItemFlags_ButtonRepeat))
                if (g.IO.MouseDownDuration[g.ActiveIdMouseButton] > 0.0f &&
                    IsMouseClicked(g.ActiveIdMouseButton,
                                   ImGuiInputFlags_Repeat, test_owner_id))
                    pressed = true;
        }
    ...
    // We report navigated and navigation-activated items as hovered but we
    // don't set g.HoveredId to not interfere with mouse.
    ...
    if (g.NavActivateDownId == id) {
        ...
        if (!nav_activated_by_inputs &&
            (item_flags & ImGuiItemFlags_ButtonRepeat)) {
            // Avoid pressing multiple keys from triggering excessive amount of
            // repeat events
            const ImGuiKeyData* key1 = GetKeyData(ImGuiKey_Space);
            const ImGuiKeyData* key2 = GetKeyData(ImGuiKey_Enter);
            const ImGuiKeyData* key3 = GetKeyData(ImGuiKey_NavGamepadActivate);
            const float t1 =
                ImMax(ImMax(key1->DownDuration, key2->DownDuration),
                      key3->DownDuration);
    ...
    if (g.ActiveId == id) {
        if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
                ...
                if ((release_in || release_anywhere) && !g.DragDropActive) {
                    ...
                    bool is_repeating_already =
                        ...
                        g.IO.MouseDownDurationPrev[mouse_button] >=
                            g.IO.KeyRepeatDelay; // Repeat mode trumps <on
                                                 // release>
                    bool is_button_avail_or_owned = TestKeyOwner(
                        MouseButtonToKey(mouse_button), test_owner_id);
            ...
            if (g.NavActivateDownId == id)
                held = true; // hovered == true not true as we are already
                             // likely hovered on direct activation.
            else
                ClearActiveID();
        }
        if (pressed)
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos) {
    ...
    // proportion of the visible region. That's in order to facilitate moving
    // the window away. (#3825) This may better be applied as a general hit-rect
    ...
    window->DrawList->AddLine(
        cross_center + ImVec2(+cross_extent, +cross_extent),
        cross_center + ImVec2(-cross_extent, -cross_extent), cross_col, 1.0f);
    window->DrawList->AddLine(
        cross_center + ImVec2(+cross_extent, -cross_extent),
        cross_center + ImVec2(-cross_extent, +cross_extent), cross_col, 1.0f);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis,
                        ImS64* p_scroll_v, ImS64 size_visible_v,
                        ImS64 size_contents_v, ImDrawFlags flags) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
        return false;
    ...
    ImRect bb = bb_frame;
    bb.Expand(ImVec2(
        -ImClamp(IM_TRUNC((bb_frame_width - 2.0f) * 0.5f), 0.0f, 3.0f),
    ...
    const float grab_h_pixels =
        ImClamp(scrollbar_size_v * ((float)size_visible_v / (float)win_size_v),
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f) {
        ...
        // called in Begin() after the calculation of ContentSize and before
        // setting up our starting position
        if (g.ScrollbarSeekMode == 0) {
            // Absolute seeking
            const float scroll_v_norm =
                ImSaturate((clicked_v_norm - g.ScrollbarClickDeltaToGrabCenter -
                            grab_h_norm * 0.5f) /
                           (1.0f - grab_h_norm));
            *p_scroll_v = (ImS64)(scroll_v_norm * scroll_max);
            ...
            if (IsMouseClicked(ImGuiMouseButton_Left, ImGuiInputFlags_Repeat) &&
                ...
                *p_scroll_v =
                    ImClamp(*p_scroll_v + (ImS64)(page_dir * size_visible_v),
                            (ImS64)0, scroll_max);
            }
        }
        ...
        // if (seek_absolute)
        //    g.ScrollbarClickDeltaToGrabCenter = clicked_v_norm - grab_v_norm -
        //    grab_h_norm * 0.5f;
    }
    ...
    if (axis == ImGuiAxis_X)
        ...
        grab_rect =
            ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x,
...
// above.
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& image_size,
...
bool ImGui::ImageButton(const char* str_id, ImTextureID user_texture_id,
                        const ImVec2& image_size, const ImVec2& uv0,
                        const ImVec2& uv1, const ImVec4& bg_col,
    ...
    return ImageButtonEx(window->GetID(str_id), user_texture_id, image_size,
                         uv0, uv1, bg_col, tint_col);
}
...
// new ImageButton() you can use PushStyleVar(ImGuiStyleVar_FramePadding,
...
bool ImGui::Checkbox(const char* label, bool* v) {
    ...
    const bool is_multi_select =
        (g.LastItemData.InFlags & ImGuiItemFlags_IsMultiSelect) != 0;
    if (!is_visible)
        if (!is_multi_select || !g.BoxSelectState.UnclipMode ||
            !g.BoxSelectState.UnclipRect.Overlaps(
                total_bb)) // Extra layer of "no logic clip" for box-select
                           // support
        {
    ...
    if (is_visible) {
        ...
        if (mixed_value) {
            ...
            RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad),
...
bool ImGui::RadioButton(const char* label, bool active) {
    ...
    const int num_segment =
        window->DrawList->_CalcCircleAutoSegmentCount(radius);
    ...
    if (style.FrameBorderSize > 0.0f) {
        ...
        window->DrawList->AddCircle(center, radius,
                                    GetColorU32(ImGuiCol_Border), num_segment,
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg,
    ...
    // The value must be animated along with time, so e.g. passing '-1.0f *
    // ImGui::GetTime()' as fraction works.
    ...
    if (is_indeterminate) {
        ...
        fill_n0 =
            ImFmod(-fraction, 1.0f) * (1.0f + fill_width_n) - fill_width_n;
    ...
    RenderRectFilledRangeH(window->DrawList, bb,
                           GetColorU32(ImGuiCol_PlotHistogram), fill_n0,
                           fill_n1, style.FrameRounding);

    // Default displaying the fraction as percentage string, but user can
    // override it Don't display text for indeterminate bars by default
    char overlay_buf[32];
    if (!is_indeterminate || overlay != NULL) {
        ...
        if (overlay_size.x > 0.0f) {
            float text_x =
                is_indeterminate
                    ? (bb.Min.x + bb.Max.x - overlay_size.x) * 0.5f
                    : ImLerp(bb.Min.x, bb.Max.x, fill_n1) + style.ItemSpacing.x;
            RenderTextClipped(ImVec2(ImClamp(text_x, bb.Min.x,
                                             bb.Max.x - overlay_size.x -
                                                 style.ItemInnerSpacing.x),
                                     bb.Min.y),
                              bb.Max, overlay, NULL, &overlay_size,
                              ImVec2(0.0f, 0.5f), &bb);
        }
    }
}
...
// FIXME-STYLE: we delayed adding as there is a larger plan to revamp the
// styling system. Because of this we currently don't provide many styling
// options for this widget (e.g. hovered/active colors are automatically
...
bool ImGui::TextLink(const char* label) {
    ...
    {
        // FIXME-STYLE: Read comments above. This widget is NOT written in the
        // same style as some earlier widgets, as we are currently
        // experimenting/planning a different styling system.
        float h, s, v;
    ...
    window->DrawList->AddLine(
        ImVec2(bb.Min.x, line_y), ImVec2(bb.Max.x, line_y),
        GetColorU32(line_colf)); // FIXME-TEXT: Underline mode.
...
void ImGui::TextLinkOpenURL(const char* label, const char* url) {
    ...
    SetItemTooltip("%s", url); // It is more reassuring for user to _always_
                               // display URL when we same as label
...
void ImGui::AlignTextToFramePadding() {
    ...
    ImGuiContext& g = *GImGui;
    window->DC.CurrLineSize.y = ImMax(window->DC.CurrLineSize.y,
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness) {
    ...
    IM_ASSERT(ImIsPowerOfTwo(
        flags & (ImGuiSeparatorFlags_Horizontal |
                 ImGuiSeparatorFlags_Vertical))); // Check that only 1 option is
                                                  // selected
    ...
    if (flags & ImGuiSeparatorFlags_Vertical) {
        ...
        // columns of a Columns() set. We currently don't need to provide the
        // same feature for tables because tables naturally have border
        // features.
        ImGuiOldColumns* columns = (flags & ImGuiSeparatorFlags_SpanAllColumns)
        ...
        // We don't provide our width to the layout so that it doesn't get feed
        // back into AutoFit
        // FIXME: This prevents ->CursorMaxPos based bounding box evaluation
        // from working (e.g. TableEndCell)
...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label,
    ...
    const float text_baseline_y =
        ImTrunc((bb.GetHeight() - label_size.y) * style.SeparatorTextAlign.y +
                0.99999f); // ImMax(padding.y, ImFloor((style.SeparatorTextSize
                           // - label_size.y) * 0.5f));
    ...
    if (label_size.x > 0.0f) {
        ...
        if (sep1_x2 > sep1_x1 && separator_thickness > 0.0f)
            window->DrawList->AddLine(ImVec2(sep1_x1, seps_y),
                                      ImVec2(sep1_x2, seps_y), separator_col,
                                      separator_thickness);
        if (sep2_x2 > sep2_x1 && separator_thickness > 0.0f)
            window->DrawList->AddLine(ImVec2(sep2_x1, seps_y),
                                      ImVec2(sep2_x2, seps_y), separator_col,
                                      separator_thickness);
        if (g.LogEnabled)
            LogSetNextTextDecoration("---", NULL);
        RenderTextEllipsis(window->DrawList, label_pos,
                           ImVec2(bb.Max.x, bb.Max.y + style.ItemSpacing.y),
                           bb.Max.x, bb.Max.x, label, label_end, &label_size);
    } else {
        if (g.LogEnabled)
            LogText("---");
...
void ImGui::SeparatorText(const char* label) {
    ...
    // - allow separator-text to be draggable items (would require a stable ID +
    ...
    // would layout a dedicated drag-item, and then we can turn this into a
    // format function.
...
bool ImGui::SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis,
    ...
    // ImGuiButtonFlags_AllowOverlap here is to allow caller of
    // SplitterBehavior() to call SetItemAllowOverlap() after the item. Nowadays
    // we would instead want to use SetNextItemAllowOverlap() before the item.
    ...
    if (bg_col & IM_COL32_A_MASK)
        window->DrawList->AddRectFilled(bb_render.Min, bb_render.Max, bg_col,
                                        0.0f);
    const ImU32 col =
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count,
    ...
    while (width_excess > 0.0f)
        for (int n = 0; n < count && width_excess > 0.0f; n++) {
            float width_to_add =
                ImMin(items[n].InitialWidth - items[n].Width, 1.0f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value,
    ...
    if (flags & ImGuiComboFlags_WidthFitPreview)
        IM_ASSERT((flags & (ImGuiComboFlags_NoPreview |
                            (ImGuiComboFlags)ImGuiComboFlags_CustomPreview)) ==
                  0);
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview)) {
        ...
        RenderTextClipped(bb.Min + style.FramePadding,
                          ImVec2(value_x2, bb.Max.y), preview_value, NULL,
                          NULL);
    }
    if (label_size.x > 0)
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb,
                            ImGuiComboFlags flags) {
    ImGuiContext& g = *GImGui;
    ...
    if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint) {
        ...
        IM_ASSERT(
            ImIsPowerOfTwo(flags & ImGuiComboFlags_HeightMask_)); // Only one
    ...
    // where Tooltips and other Popups are calling
    // FindBestWindowPosForPopupEx()?
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive) {
            // Always override 'AutoPosLastDirection' to not leave a chance for
            // a past value to affect us.
...
void ImGui::EndComboPreview() {
    ...
    // will store in ImDrawCmd in the future
    ...
    window->DC.CursorMaxPos =
        ImMax(window->DC.CursorMaxPos, preview_data->BackupCursorMaxPos);
...
bool ImGui::Combo(const char* label, int* current_item,
    ...
    if (popup_max_height_in_items != -1 &&
        ...
        SetNextWindowSizeConstraints(
            ImVec2(0, 0), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(
...
bool ImGui::Combo(const char* label, int* current_item,
    ...
    const bool value_changed =
        Combo(label, current_item, Items_ArrayGetter, (void*)items, items_count,
              height_in_items);
    return value_changed;
}
...
bool ImGui::ListBox(const char* label, int* current_item,
    ...
    ImGuiGetNameFromIndexOldToNewCallbackData old_to_new_data = {user_data,
                                                                 old_getter};
...
bool ImGui::Combo(const char* label, int* current_item,
    ...
    ImGuiGetNameFromIndexOldToNewCallbackData old_to_new_data = {user_data,
                                                                 old_getter};
    return Combo(label, current_item, ImGuiGetNameFromIndexOldToNewCallback,
...
void ImGui::DataTypeApplyOp(ImGuiDataType data_type, int op, void* output,
    ...
    case ImGuiDataType_S16:
        if (op == '+') {
            *(ImS16*)output =
                ImAddClampOverflow(*(const ImS16*)arg1, *(const ImS16*)arg2,
                                   IM_S16_MIN, IM_S16_MAX);
        }
        if (op == '-') {
            *(ImS16*)output =
                ImSubClampOverflow(*(const ImS16*)arg1, *(const ImS16*)arg2,
                                   IM_S16_MIN, IM_S16_MAX);
        }
        return;
    case ImGuiDataType_U16:
        if (op == '+') {
            *(ImU16*)output =
                ImAddClampOverflow(*(const ImU16*)arg1, *(const ImU16*)arg2,
                                   IM_U16_MIN, IM_U16_MAX);
        }
        if (op == '-') {
            *(ImU16*)output =
                ImSubClampOverflow(*(const ImU16*)arg1, *(const ImU16*)arg2,
                                   IM_U16_MIN, IM_U16_MAX);
        }
        return;
    case ImGuiDataType_S32:
        if (op == '+') {
            *(ImS32*)output =
                ImAddClampOverflow(*(const ImS32*)arg1, *(const ImS32*)arg2,
                                   IM_S32_MIN, IM_S32_MAX);
        }
        if (op == '-') {
            *(ImS32*)output =
                ImSubClampOverflow(*(const ImS32*)arg1, *(const ImS32*)arg2,
                                   IM_S32_MIN, IM_S32_MAX);
        }
        return;
    case ImGuiDataType_U32:
        if (op == '+') {
            *(ImU32*)output =
                ImAddClampOverflow(*(const ImU32*)arg1, *(const ImU32*)arg2,
                                   IM_U32_MIN, IM_U32_MAX);
        }
        if (op == '-') {
            *(ImU32*)output =
                ImSubClampOverflow(*(const ImU32*)arg1, *(const ImU32*)arg2,
                                   IM_U32_MIN, IM_U32_MAX);
        }
        return;
    case ImGuiDataType_S64:
        if (op == '+') {
            *(ImS64*)output =
                ImAddClampOverflow(*(const ImS64*)arg1, *(const ImS64*)arg2,
                                   IM_S64_MIN, IM_S64_MAX);
        }
        if (op == '-') {
            *(ImS64*)output =
                ImSubClampOverflow(*(const ImS64*)arg1, *(const ImS64*)arg2,
                                   IM_S64_MIN, IM_S64_MAX);
        }
        return;
    case ImGuiDataType_U64:
        if (op == '+') {
            *(ImU64*)output =
                ImAddClampOverflow(*(const ImU64*)arg1, *(const ImU64*)arg2,
                                   IM_U64_MIN, IM_U64_MAX);
        }
        if (op == '-') {
            *(ImU64*)output =
                ImSubClampOverflow(*(const ImU64*)arg1, *(const ImU64*)arg2,
                                   IM_U64_MIN, IM_U64_MAX);
        }
        return;
...
bool ImGui::DataTypeApplyFromText(const char* buf, ImGuiDataType data_type,
    ...
    // - In theory could treat empty format as using default, but this would
    // only cover rare/bizarre case of using InputScalar() + integer + format
    // string without %.
    char format_sanitized[32];
    if (data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double)
        format = type_info->ScanFmt;
    else
        format = ImParseFormatSanitizeForScanning(
            format, format_sanitized, IM_ARRAYSIZE(format_sanitized));
    ...
    if (type_info->Size < 4) {
        if (data_type == ImGuiDataType_S8)
            ...
            *(ImS16*)p_data =
                (ImS16)ImClamp(v32, (int)IM_S16_MIN, (int)IM_S16_MAX);
        else if (data_type == ImGuiDataType_U16)
            *(ImU16*)p_data =
                (ImU16)ImClamp(v32, (int)IM_U16_MIN, (int)IM_U16_MAX);
        else
            IM_ASSERT(0);
    }
...
bool ImGui::DataTypeClamp(ImGuiDataType data_type, void* p_data,
    ...
    case ImGuiDataType_U8:
        return DataTypeClampT<ImU8>((ImU8*)p_data, (const ImU8*)p_min,
                                    (const ImU8*)p_max);
    case ImGuiDataType_S16:
        return DataTypeClampT<ImS16>((ImS16*)p_data, (const ImS16*)p_min,
                                     (const ImS16*)p_max);
    case ImGuiDataType_U16:
        return DataTypeClampT<ImU16>((ImU16*)p_data, (const ImU16*)p_min,
                                     (const ImU16*)p_max);
    case ImGuiDataType_S32:
        return DataTypeClampT<ImS32>((ImS32*)p_data, (const ImS32*)p_min,
                                     (const ImS32*)p_max);
    case ImGuiDataType_U32:
        return DataTypeClampT<ImU32>((ImU32*)p_data, (const ImU32*)p_min,
                                     (const ImU32*)p_max);
    case ImGuiDataType_S64:
        return DataTypeClampT<ImS64>((ImS64*)p_data, (const ImS64*)p_min,
                                     (const ImS64*)p_max);
    case ImGuiDataType_U64:
        return DataTypeClampT<ImU64>((ImU64*)p_data, (const ImU64*)p_min,
                                     (const ImU64*)p_max);
    case ImGuiDataType_Float:
...
TYPE ImGui::RoundScalarWithFormatT(const char* format, ImGuiDataType data_type,
                                   TYPE v) {
    IM_UNUSED(data_type);
    IM_ASSERT(data_type == ImGuiDataType_Float ||
    ...
    ImParseFormatSanitizeForPrinting(fmt_start, fmt_sanitized,
                                     IM_ARRAYSIZE(fmt_sanitized));
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed,
    ...
    const bool is_floating_point = (data_type == ImGuiDataType_Float) ||
                                   (data_type == ImGuiDataType_Double);
    ...
    if (g.ActiveIdSource == ImGuiInputSource_Mouse && IsMousePosValid() &&
        ...
        const bool tweak_slow =
            IsKeyDown((g.NavInputSource == ImGuiInputSource_Gamepad)
        ...
        const float tweak_factor = tweak_slow   ? 1.0f / 10.0f
                                   : tweak_fast ? 10.0f
                                                : 1.0f;
        adjust_delta = GetNavTweakPressedAmount(axis) * tweak_factor;
        v_speed =
            ImMax(v_speed, GetMinimumStepAtDecimalPrecision(decimal_precision));
    }
    adjust_delta *= v_speed;

    // For vertical drag we currently assume that Up=higher value (like we do
    // with vertical sliders). This may become a parameter.
    if (axis == ImGuiAxis_Y)
    ...
    if (is_logarithmic) {
        // When using logarithmic sliders, we need to clamp to avoid hitting
        // zero, but our choice of clamp value greatly affects slider precision.
        // We attempt to use the specified precision to estimate a good lower
        // bound.
        const int decimal_precision =
            is_floating_point ? ImParseFormatPrecision(format, 3) : 1;
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v,
                         ...
                         const char* format, ImGuiSliderFlags flags) {
    // Read imgui.cpp "API BREAKING CHANGES" section for 1.78 if you hit this
    // assert.
    IM_ASSERT((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) &&
              "Invalid ImGuiSliderFlags flags! Has the legacy 'float power' "
              "argument been mistakenly cast to flags? Call function with "
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data,
    ...
    if (!temp_input_is_active) {
        ...
        const bool clicked =
            hovered && IsMouseClicked(0, ImGuiInputFlags_None, id);
...
bool ImGui::DragScalarN(const char* label, ImGuiDataType data_type,
    ...
    for (int i = 0; i < components; i++) {
        ...
        value_changed |= DragScalar("", data_type, p_data, v_speed, p_min,
                                    p_max, format, flags);
        PopID();
        PopItemWidth();
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min,
    ...
    value_changed |= DragScalar("##max", ImGuiDataType_Float, v_current_max,
                                v_speed, &max_min, &max_max,
...
bool ImGui::DragInt4(const char* label, int v[4], float v_speed, int v_min,
                     int v_max, const char* format, ImGuiSliderFlags flags) {
    return DragScalarN(label, ImGuiDataType_S32, v, 4, v_speed, &v_min, &v_max,
                       format, flags);
}
...
float ImGui::ScaleRatioFromValueT(ImGuiDataType data_type, TYPE v, TYPE v_min,
    ...
    if (is_logarithmic) {
        ...
        // Awkward special cases - we need ranges of the form (-100 .. 0) to
        // convert to (-100 .. -epsilon), not (-100 .. epsilon)
        ...
        if (v_clamped <= v_min_fudged)
            result = 0.0f; // Workaround for values that are in-range but below
                           // our fudge
        else if (v_clamped >= v_max_fudged)
            result = 1.0f; // Workaround for values that are in-range but above
                           // our fudge
        else if ((v_min * v_max) <
                 0.0f) // Range crosses zero, so split into two portions
        {
            float zero_point_center =
                ...
                ((float)v_max -
                 (float)
                     v_min); // The zero point in parametric space.  There's an
                             // argument we should take the logarithmic nature
                             // into account when calculating this, but for now
                             // this should do (and the most common case of a
            ...
            result =
                1.0f - (float)(ImLog(-(FLOATTYPE)v_clamped / -v_max_fudged) /
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min,
    ...
    if (is_logarithmic) {
        ...
        // Awkward special case - we need ranges of the form (-100 .. 0) to
        // convert to (-100 .. -epsilon), not (-100 .. epsilon)
        if ((v_max == 0.0f) && (v_min < 0.0f))
            v_max_fudged = -logarithmic_zero_epsilon;

        float t_with_flip =
            flipped ? (1.0f - t) : t; // t, but flipped if necessary to account
                                      // for us flipping the range
            ...
            0.0f) // Range crosses zero, so we have to do this in two parts
        {
            ...
            result = (TYPE) -
                     (-v_max_fudged * ImPow(-v_min_fudged / -v_max_fudged,
                                            (FLOATTYPE)(1.0f - t_with_flip)));
        else
            result = (TYPE)(v_min_fudged * ImPow(v_max_fudged / v_min_fudged,
        ...
        if (is_floating_point) {
            ...
            // - For integer values we want the clicking position to match the
            // grab box so we round above
            //   This code is carefully tuned to work with large values (e.g.
            //   high ranges of U64) while preserving this property..
            // - Not doing a *1.0 multiply at the end of a range as it tends to
            // be lossy. While absolute aiming at a large s64/u64
            //   range is going to be imprecise anyway, with this check we at
            //   least make the edge values matches expected limits.
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id,
    ...
    if (is_logarithmic) {
        ...
        zero_deadzone_halfsize =
            (style.LogSliderDeadzone * 0.5f) / ImMax(slider_usable_sz, 1.0f);
    }
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
            if (!g.IO.MouseDown[0]) {
                ...
                if (g.ActiveIdIsJustActivated) {
                    ...
                    if (axis == ImGuiAxis_Y)
                        grab_t = 1.0f - grab_t;
                    const float grab_pos = ImLerp(
                        slider_usable_pos_min, slider_usable_pos_max, grab_t);
                    ...
                    g.SliderGrabClickOffset =
                        (clicked_around_grab && is_floating_point)
                            ? mouse_abs_pos - grab_pos
                            : 0.0f;
                }
                if (slider_usable_sz > 0.0f)
                    clicked_t =
                        ImSaturate((mouse_abs_pos - g.SliderGrabClickOffset -
            ...
            if (input_delta != 0.0f) {
                ...
                const int decimal_precision =
                    is_floating_point ? ImParseFormatPrecision(format, 3) : 0;
                if (decimal_precision > 0) {
                    input_delta /= 100.0f; // Gamepad/keyboard tweak speeds in %
                                           // of slider bounds
                    if (tweak_slow)
                        input_delta /= 10.0f;
                } else {
                    if ((v_range_f >= -100.0f && v_range_f <= 100.0f &&
                        ...
                        input_delta = ((input_delta < 0.0f) ? -1.0f : +1.0f) /
                                      v_range_f; // Gamepad/keyboard tweak
                                                 // speeds in integer steps
                    else
                        input_delta /= 100.0f;
                }
                if (tweak_fast)
            ...
            if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
                ...
                if ((clicked_t >= 1.0f && delta > 0.0f) ||
                    (clicked_t <= 0.0f &&
                     delta < 0.0f)) // This is to avoid applying the saturation
                                    // when already past the limits
                {
                    set_new_value = false;
                    g.SliderCurrentAccum =
                        0.0f; // If pushing up against the limits, don't
                    ...
                    // Calculate what our "new" clicked_t will be, and thus how
                    // far we actually moved the slider, and subtract this from
                    // the accumulator
                    TYPE v_new =
                    ...
                    if (is_floating_point &&
                        ...
                        v_new = RoundScalarWithFormatT<TYPE>(format, data_type,
                                                             v_new);
                    float new_clicked_t =
                        ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(
                    ...
                    if (delta > 0)
                        g.SliderCurrentAccum -=
                            ImMin(new_clicked_t - old_clicked_t, delta);
                    else
                        g.SliderCurrentAccum -=
                            ImMax(new_clicked_t - old_clicked_t, delta);
                }
    ...
    if (slider_sz < 1.0f) {
        *out_grab_bb = ImRect(bb.Min, bb.Min);
        ...
        if (axis == ImGuiAxis_X)
            *out_grab_bb =
                ImRect(grab_pos - grab_sz * 0.5f, bb.Min.y + grab_padding,
                       grab_pos + grab_sz * 0.5f, bb.Max.y - grab_padding);
        else
            *out_grab_bb =
                ImRect(bb.Min.x + grab_padding, grab_pos - grab_sz * 0.5f,
...
bool ImGui::VSliderFloat(const char* label, const ImVec2& size, float* v,
                         float v_min, float v_max, const char* format,
                         ImGuiSliderFlags flags) {
    return VSliderScalar(label, size, ImGuiDataType_Float, v, &v_min, &v_max,
                         format, flags);
}
...
const char* ImParseFormatTrimDecorations(const char* fmt, char* buf,
    ...
    if (fmt_end[0] == 0) // If we only have leading decoration, we don't need to
                         // copy the data.
        return fmt_start;
...
void ImParseFormatSanitizeForPrinting(const char* fmt_in, char* fmt_out,
                                      size_t fmt_out_size) {
    const char* fmt_end = ImParseFormatFindEnd(fmt_in);
    IM_UNUSED(fmt_out_size);
    IM_ASSERT((size_t)(fmt_end - fmt_in + 1) <
              fmt_out_size); // Format is too long, let us know if this happens
                             // to you!
...
bool ImGui::TempInputText(const ImRect& bb, ImGuiID id, const char* label,
    ...
    if (init) {
        // First frame we started displaying the InputText widget, we expect it
        // to take the active id.
...
bool ImGui::TempInputScalar(const ImRect& bb, ImGuiID id, const char* label,
    ...
    char data_buf[32];
    format =
        ImParseFormatTrimDecorations(format, fmt_buf, IM_ARRAYSIZE(fmt_buf));
    if (format[0] == 0)
        format = type_info->PrintFmt;
    DataTypeFormatString(data_buf, IM_ARRAYSIZE(data_buf), data_type, p_data,
                         format);
    ImStrTrimBlanks(data_buf);

    ImGuiInputTextFlags flags =
        ImGuiInputTextFlags_AutoSelectAll |
        (ImGuiInputTextFlags)ImGuiInputTextFlags_NoMarkEdited |
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type,
    ...
    flags |= ImGuiInputTextFlags_AutoSelectAll |
             (ImGuiInputTextFlags)
                 ImGuiInputTextFlags_NoMarkEdited; // We call MarkItemEdited()
    ...
    if (p_step == NULL) {
        if (InputText(label, buf, IM_ARRAYSIZE(buf), flags))
            value_changed = DataTypeApplyFromText(
                ...
                (flags & ImGuiInputTextFlags_ParseEmptyRefVal) ? p_data_default
                                                               : NULL);
    } else {
        const float button_size = GetFrameHeight();

        BeginGroup(); // The only purpose of the group here is to allow the
                      // caller to query item data e.g. IsItemActive()
        PushID(label);
        SetNextItemWidth(
            ImMax(1.0f, CalcItemWidth() -
...
bool ImGui::InputScalarN(const char* label, ImGuiDataType data_type,
    ...
    for (int i = 0; i < components; i++) {
        ...
        if (i > 0)
            SameLine(0, g.Style.ItemInnerSpacing.x);
        value_changed |= InputScalar("", data_type, p_data, p_step, p_step_fast,
                                     format, flags);
        PopID();
        PopItemWidth();
        p_data = (void*)((char*)p_data + type_size);
...
bool ImGui::InputFloat2(const char* label, float v[2], const char* format,
                        ImGuiInputTextFlags flags) {
    return InputScalarN(label, ImGuiDataType_Float, v, 2, NULL, NULL, format,
                        flags);
}
...
bool ImGui::InputFloat4(const char* label, float v[4], const char* format,
                        ImGuiInputTextFlags flags) {
    return InputScalarN(label, ImGuiDataType_Float, v, 4, NULL, NULL, format,
                        flags);
}
...
bool ImGui::InputDouble(const char* label, double* v, double step,
    ...
    return InputScalar(label, ImGuiDataType_Double, (void*)v,
                       ...
                       (void*)(step_fast > 0.0 ? &step_fast : NULL), format,
                       flags);
}
...
bool ImGui::InputTextWithHint(const char* label, const char* hint, char* buf,
                              ...
                              void* user_data) {
    IM_ASSERT(!(flags &
                ImGuiInputTextFlags_Multiline)); // call InputTextMultiline() or
...
static ImVec2
InputTextCalcTextSizeW(ImGuiContext* ctx, const ImWchar* text_begin,
                       const ImWchar* text_end, const ImWchar** remaining,
    ...
    if (out_offset)
        *out_offset = ImVec2(
            line_width,
            text_size.y + line_height); // offset allow for the possibility of
                                        // sitting after a trailing \n
...
static float STB_TEXTEDIT_GETWIDTH(ImGuiInputTextState* obj, int line_start_idx,
                                   int char_idx) {
...
static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r, ImGuiInputTextState* obj,
    ...
    const ImVec2 size = InputTextCalcTextSizeW(obj->Ctx, text + line_start_idx,
                                               text + obj->CurLenW,
...
static int is_word_boundary_from_right(ImGuiInputTextState* obj, int idx) {
    // When ImGuiInputTextFlags_Password is set, we don't want actions such as
...
static bool STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState* obj, int pos,
    ...
    if (new_text_len + text_len + 1 > obj->TextW.Size) {
        ...
        obj->TextW.resize(
            text_len + ImClamp(new_text_len * 4, 32, ImMax(256, new_text_len)) +
            1);
    }
...
static bool InputTextFilterCharacter(ImGuiContext* ctx, unsigned int* p_char,
    ...
    if (c < 0x20) {
        ...
        pass |= (c == '\n') &&
                (flags & ImGuiInputTextFlags_Multiline) !=
                    0; // Note that an Enter KEY will emit \r and be ignored (we
                       // poll for KEY in InputText() code)
        pass |= (c == '\t') && (flags & ImGuiInputTextFlags_AllowTabInput) != 0;
        if (!pass)
            return false;
        apply_named_filters = false; // Override named filters below so newline
                                     // and tabs can still be inserted.
    }
    ...
    if (apply_named_filters &&
        ...
        // e.g. ',' instead of '.'. The standard mandate that programs starts in
        // the "C" locale where the decimal point is '.'. We don't really intend
        // to provide widespread support for it, but out of empathy for people
        // stuck with using odd API, we support the bare minimum aka overriding
        // the decimal point. Change the default decimal_point with:
        //   ImGui::GetIO()->PlatformLocaleDecimalPoint =
        //   *localeconv()->decimal_point;
        // Users of non-default decimal point (in particular ',') may be
        // affected by word-selection logic
        ...
        // While this is mostly convenient, this has the side-effect for
        // uninformed users accidentally inputting full-width characters that
        // they may scratch their head as to why it works in numerical fields vs
        // in generic text fields it would require support in the font.
...
static void InputTextReconcileUndoStateAfterUserCallback(
    ImGuiInputTextState* state, const char* new_buf_a, int new_length_a) {
    ImGuiContext& g = *GImGui;
    const ImWchar* old_buf = state->TextW.Data;
    const int old_length = state->CurLenW;
    const int new_length =
        ImTextCountCharsFromUtf8(new_buf_a, new_buf_a + new_length_a);
    g.TempBuffer.reserve_discard((new_length + 1) * sizeof(ImWchar));
    ImWchar* new_buf = (ImWchar*)(void*)g.TempBuffer.Data;
    ImTextStrFromUtf8(new_buf, new_length + 1, new_buf_a,
                      new_buf_a + new_length_a);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf,
    ...
    IM_ASSERT(
        !((flags & ImGuiInputTextFlags_CallbackHistory) &&
          (flags &
           ImGuiInputTextFlags_Multiline))); // Can't use both together (they
                                             // both use up/down keys)
    IM_ASSERT(
    ...
    if (is_multiline) {
        ...
        // Prevent NavActivation from Tabbing when our widget accepts Tab
        // inputs: this allows cycling through widgets without stopping.
        if (g.NavActivateId == id &&
            (g.NavActivateFlags & ImGuiActivateFlags_FromTabbing) &&
        ...
        PushStyleVar(ImGuiStyleVar_WindowPadding,
                     ImVec2(0, 0)); // Ensure no clip rect so mouse hover can
                                    // reach FramePadding edges
        bool child_visible =
            BeginChildEx(label, id, frame_bb.GetSize(), ImGuiChildFlags_Border,
        ...
        // Support for internal ImGuiInputTextFlags_MergedItem flag, which could
        // be redesigned as an ItemFlags if needed (with test performed in
        // ItemAdd)
        ItemSize(total_bb, style.FramePadding.y);
    ...
    const bool init_reload_from_user_buf =
        (state != NULL && state->ReloadUserBuf);
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs ||
        ...
        // Backup state of deactivating item so they'll have a chance to do a
        // write to output buffer on the same frame they report
        // IsItemDeactivatedAfterEdit (#4714)
        InputTextDeactivateHook(state->ID);

        // From the moment we focused we are normally ignoring the content of
        // 'buf' (unless we are in read-only mode)
        const int buf_len = (int)strlen(buf);
        if (!init_reload_from_user_buf) {
            // Take a copy of the initial buffer value.
            state->InitialTextA.resize(
                buf_len + 1); // UTF-8. we use +1 to make sure that .Data is
        ...
        // widget
        // FIXME: Since we reworked this on 2022/06, may want to differentiate
        ...
        state->CurLenA =
            (int)(buf_end - buf); // We can't get the result from ImStrncpy()
    ...
    if (g.ActiveId == id) {
        // Declare some inputs, the other are registered and polled via
        // Shortcut() routing system.
        if (user_clicked)
            SetKeyOwner(ImGuiKey_MouseLeft, id);
        g.ActiveIdUsingNavDirMask |=
            (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
        ...
        // FIXME: May be a problem to always steal Alt on OSX, would ideally
        // still allow an uninterrupted Alt down-up to toggle menu
        if (is_osx)
    ...
    if (g.ActiveId == id) {
        ...
        // elements unless we are interacting right now with the widget. Down
        // the line we should have a cleaner library-wide concept of Selected vs
        // Active.
        ...
        if (select_all) {
            ...
            if ((multiclick_count % 2) == 0) {
                ...
                // We always use the "Mac" word advance for double-click select
                // vs CTRL+Right which use the platform dependent variant:
                // FIXME: There are likely many ways to improve this behavior,
                // but there's no "right" behavior (depends on use-case,
                // software, OS)
                const bool is_bol = (state->Stb.cursor == 0) ||
                                    ImStb::STB_TEXTEDIT_GETCHAR(
                                        state, state->Stb.cursor - 1) == '\n';
                ...
                state->Stb.cursor = ImStb::STB_TEXTEDIT_MOVEWORDRIGHT_MAC(
                    state, state->Stb.cursor);
                state->Stb.select_end = state->Stb.cursor;
                ImStb::stb_textedit_clamp(state, &state->Stb);
            } else {
                // Triple-click: Select line
                const bool is_eol = ImStb::STB_TEXTEDIT_GETCHAR(
                                        state, state->Stb.cursor) == '\n';
        ...
        // We expect backends to emit a Tab key but some also emit a Tab
        // character which we ignore (#2467, #1336) (For Tab and Enter:
        // Win32/SFML/Allegro are sending both keys and chars, GLFW and SDL are
        // only sending keys. For Space they all send all threes)
        if ((flags & ImGuiInputTextFlags_AllowTabInput) && !is_readonly) {
            ...
            /*
            if (Shortcut(ImGuiKey_Tab | ImGuiMod_Shift, ImGuiInputFlags_Repeat,
            id))
            {
            }
            */
        ...
        // Process regular text input (before we check for Return because using
        // some IME will effectively send a Return?) We ignore CTRL inputs, but
        // need to allow ALT+CTRL as some keyboards (e.g. German) use AltGR
        // (which _is_ Alt+Ctrl) to input certain characters.
        const bool ignore_char_inputs =
            (io.KeyCtrl && !io.KeyAlt) || (is_osx && io.KeyCtrl);
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
        ...
        const bool is_wordmove_key_down =
            is_osx ? io.KeyAlt
                   : io.KeyCtrl; // OS X style: Text editing cursor movement
        ...
        // Using Shortcut() with ImGuiInputFlags_RouteFocused (default policy)
        // to allow routing operations for other code (e.g. calling window
        // trying to use CTRL+A and CTRL+B: formet would be handled by
        // InputText) Otherwise we could simply assume that we own the keys as
        // we are active.
        const ImGuiInputFlags f_repeat = ImGuiInputFlags_Repeat;
        const bool is_cut =
            (Shortcut(ImGuiMod_Ctrl | ImGuiKey_X, f_repeat, id) ||
             Shortcut(ImGuiMod_Shift | ImGuiKey_Delete, f_repeat, id)) &&
            !is_readonly && !is_password &&
            (!is_multiline || state->HasSelection());
        const bool is_copy =
            (Shortcut(ImGuiMod_Ctrl | ImGuiKey_C, 0, id) ||
             Shortcut(ImGuiMod_Ctrl | ImGuiKey_Insert, 0, id)) &&
            !is_password && (!is_multiline || state->HasSelection());
        const bool is_paste =
            (Shortcut(ImGuiMod_Ctrl | ImGuiKey_V, f_repeat, id) ||
             Shortcut(ImGuiMod_Shift | ImGuiKey_Insert, f_repeat, id)) &&
            !is_readonly;
        const bool is_undo =
            (Shortcut(ImGuiMod_Ctrl | ImGuiKey_Z, f_repeat, id)) &&
            !is_readonly && is_undoable;
        const bool is_redo =
            (Shortcut(ImGuiMod_Ctrl | ImGuiKey_Y, f_repeat, id) ||
             (is_osx && Shortcut(ImGuiMod_Ctrl | ImGuiMod_Shift | ImGuiKey_Z,
                                 f_repeat, id))) &&
            !is_readonly && is_undoable;
        const bool is_select_all = Shortcut(ImGuiMod_Ctrl | ImGuiKey_A, 0, id);

        // We allow validate/cancel with Nav source (gamepad) to makes it easier
        // to undo an accidental NavInput press with no keyboard wired, but
        ...
        const bool is_enter_pressed = IsKeyPressed(ImGuiKey_Enter, true) ||
        ...
        const bool is_cancel =
            ...
            (nav_gamepad_active &&
             Shortcut(ImGuiKey_NavGamepadCancel, f_repeat, id));

        // FIXME: Should use more Shortcut() and reduce
        // IsKeyPressed()+SetKeyOwner(), but requires modifiers combination to
        // be taken account of.
        // FIXME-OSX: Missing support for Alt(option)+Right/Left = go to end of
        // line, or next line if already in end of line.
        if (IsKeyPressed(ImGuiKey_LeftArrow)) {
            ...
            if (!state->HasSelection()) {
                // OSX doesn't seem to have Super+Delete to delete until
                // end-of-line, so we don't emulate that (as opposed to
                // Super+Backspace)
            ...
            if (io.SetClipboardTextFn) {
                ...
                const int clipboard_data_len =
                    ImTextCountUtf8BytesFromStr(state->TextW.Data + ib,
                                                state->TextW.Data + ie) +
                    1;
                char* clipboard_data =
                    (char*)IM_ALLOC(clipboard_data_len * sizeof(char));
                ImTextStrToUtf8(clipboard_data, clipboard_data_len,
            ...
            if (const char* clipboard = GetClipboardText()) {
                ...
                for (const char* s = clipboard; *s != 0;) {
                    ...
                    if (!InputTextFilterCharacter(&g, &c, flags, callback,
                                                  callback_user_data, true))
                        continue;
                    clipboard_filtered[clipboard_filtered_len++] = (ImWchar)c;
                }
                clipboard_filtered[clipboard_filtered_len] = 0;
                if (clipboard_filtered_len > 0) // If everything was filtered,
                                                // ignore the pasting operation
                {
    ...
    if (g.ActiveId == id) {
        ...
        if (revert_edit && !is_readonly) {
            if (flags & ImGuiInputTextFlags_EscapeClearsAll) {
                ...
                // initial value changes the current buffer contents. Push
                // records into the undo stack so we can CTRL+Z the revert
                // operation itself
                ...
                if (apply_new_text_length > 0) {
                    w_text.resize(ImTextCountCharsFromUtf8(
                                      apply_new_text,
        ...
        if (!is_readonly) {
            ...
            ImTextStrToUtf8(state->TextA.Data, state->TextA.Size,
                            state->TextW.Data, NULL);
        }

        // When using 'ImGuiInputTextFlags_EnterReturnsTrue' as a special case
        // we reapply the live buffer back to the input buffer before clearing
        // ActiveId, even though strictly speaking it wasn't modified on this
        // frame. If we didn't do that, code like InputInt() with
        // ImGuiInputTextFlags_EnterReturnsTrue would fail. This also allows the
        // user to use InputText() with ImGuiInputTextFlags_EnterReturnsTrue
        // without maintaining any user-side storage (please note that if you
        // use this property along ImGuiInputTextFlags_CallbackResize you can
        // end up with your temporary string object unnecessarily allocating
        // once a frame, either store your string data, either if you don't then
        // don't use ImGuiInputTextFlags_CallbackResize).
        const bool apply_edit_back_to_user_buffer =
            !revert_edit ||
            (validated && (flags & ImGuiInputTextFlags_EnterReturnsTrue) != 0);
        if (apply_edit_back_to_user_buffer) {
            // Apply new value immediately - copy modified buffer back
            // Note that as soon as the input box is active, the in-widget value
            // gets priority over any underlying modification of the input
            // buffer
            // FIXME: We actually always render 'buf' when calling
            // DrawList->AddText, making the comment above incorrect.
            // FIXME-OPT: CPU waste to do this every time the widget is active,
            // should mark dirty state from the stb_textedit callbacks.
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
                ...
                // The reason we specify the usage semantic (Completion/History)
                // is that Completion needs to disable keyboard TABBING at the
                // moment.
                ...
                } else if ((flags & ImGuiInputTextFlags_CallbackEdit) &&
                           state->Edited) {
                ...
                if (event_flag) {
                    ...
                    // We have to convert from wchar-positions to
                    // UTF-8-positions, which can be pretty slow (an incentive
                    // to ditch the ImWchar buffer, see
                    // https://github.com/nothings/stb/issues/188)
                    ...
                    callback_buf =
                        is_readonly
                            ? buf
                            : state->TextA
                                  .Data; // Pointer may have been invalidated by
                                         // a resize callback
                    ...
                    if (buf_dirty) {
                        ...
                        IM_ASSERT(
                            ...
                            (int)strlen(
                                callback_data
                                    .Buf)); // You need to maintain BufTextLen
                        ...
                        if (callback_data.BufTextLen >
                            ...
                            state->TextW.resize(
                                state->TextW.Size +
                                (callback_data.BufTextLen -
                                 backup_current_text_length)); // Worse case
                                                               // scenario
                                                               // resize
                        state->CurLenW = ImTextStrFromUtf8(
                            state->TextW.Data, state->TextW.Size,
    ...
    // Handle reapplying final data on deactivation (see
    // InputTextDeactivateHook() for details)
    if (g.InputTextDeactivatedState.ID == id) {
        if (g.ActiveId != id && IsItemDeactivatedAfterEdit() && !is_readonly &&
            ...
            value_changed = true;
            // IMGUI_DEBUG_LOG("InputText(): apply Deactivated data for 0x%08X:
            // \"%.*s\".\n", id, apply_new_text_length, apply_new_text);
        }
        g.InputTextDeactivatedState.ID = 0;
    }

    // Copy result to user buffer. This can currently only happen when
    // (g.ActiveId == id)
    if (apply_new_text != NULL) {
        ...
        if (is_resizable) {
            ...
            apply_new_text_length =
                ImMin(callback_data.BufTextLen, buf_size - 1);
    ...
    // carriage return, which would makes ImFont::RenderText() reserve too many
    // vertices and probably crash. Avoid it altogether. Note that we only use
    // this limit on single-line InputText(), so a pathologically large line on
    ...
    if (render_cursor || render_selection) {
        ...
        // We are attempting to do most of that in **one main pass** to minimize
        // the computation cost (non-negligible for large amount of text) + 2nd
        // pass for selection rendering (we could merge them by an extra
        // refactoring effort)
...

        {
            ...
            // In multi-line mode, we never exit the loop until all lines are
            // counted, so add one extra to the searches_remaining counter.
            searches_remaining += is_multiline ? 1 : 0;
            int line_count = 0;
            // for (const ImWchar* s = text_begin; (s = (const
            // ImWchar*)wcschr((const wchar_t*)s, (wchar_t)'\n')) != NULL; s++)
            // // FIXME-OPT: Could use this when wchar_t are 16-bit
            ...
            cursor_offset.x =
                InputTextCalcTextSizeW(
                    &g, ImStrbolW(searches_input_ptr[0], text_begin),
            ...
            if (searches_result_line_no[1] >= 0) {
                select_start_offset.x =
                    InputTextCalcTextSizeW(
                        &g, ImStrbolW(searches_input_ptr[1], text_begin),
                        searches_input_ptr[1])
                        .x;
                select_start_offset.y = searches_result_line_no[1] * g.FontSize;
            }

            // Store text height (note that we haven't calculated text width at
            // all, see GitHub issues #383, #1224)
            if (is_multiline)
        ...
        if (render_cursor && state->CursorFollow) {
            ...
            if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll)) {
                ...
                if (cursor_offset.x < state->ScrollX)
                    state->ScrollX = IM_TRUNC(
                        ImMax(0.0f, cursor_offset.x - scroll_increment_x));
                else if (cursor_offset.x - visible_width >= state->ScrollX)
                    state->ScrollX = IM_TRUNC(cursor_offset.x - visible_width +
                                              scroll_increment_x);
            } else {
                state->ScrollX = 0.0f;
            }
            ...
            if (is_multiline) {
                ...
                const float scroll_max_y = ImMax(
                    (text_size.y + style.FramePadding.y * 2.0f) - inner_size.y,
                    0.0f);
                scroll_y = ImClamp(scroll_y, 0.0f, scroll_max_y);
                draw_pos.y += (draw_window->Scroll.y -
                               scroll_y); // Manipulate cursor pos immediately
                                          // avoid a frame of lag
        ...
        if (render_selection) {
            ...
            float bg_offy_up =
                is_multiline ? 0.0f
                             : -1.0f; // FIXME: those offsets should be part of
                                      // the style? they don't play so well with
            ...
            for (const ImWchar* p = text_selected_begin;
                ...
                if (rect_pos.y < clip_rect.y) {
                    ...
                    ImVec2 rect_size = InputTextCalcTextSizeW(
                        &g, p, text_selected_end, &p, NULL, true);
                    if (rect_size.x <= 0.0f)
                        rect_size.x = IM_TRUNC(
                            g.Font->GetCharAdvance((ImWchar)' ') *
                            0.50f); // So we can see selected empty lines
                    ImRect rect(rect_pos +
                                    ImVec2(0.0f, bg_offy_up - g.FontSize),
                                rect_pos + ImVec2(rect_size.x, bg_offy_dn));
                    rect.ClipWith(clip_rect);
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max,
                                                             bg_color);
                }
        ...
        // pathological cases (e.g. single-line 1 MB string) which would make
        // ImDrawList crash.
        if (is_multiline ||
        ...
        if (render_cursor) {
            ...
            ImVec2 cursor_screen_pos =
                ImTrunc(draw_pos + cursor_offset - draw_scroll);
            ...
            // Notify OS of text input position for advanced IME (-1 x offset so
            // that Windows IME can cover our cursor. Bit of an extra nicety.)
            if (!is_readonly) {
        ...
        if (is_multiline)
            text_size =
                ImVec2(inner_size.x, InputTextCalcTextLenAndLineCount(
    ...
    if (is_multiline) {
        // For focus requests to work on our multiline we need to ensure our
        // child ItemAdd() call specifies the ImGuiItemFlags_Inputable (ref
        // issue #4761)...
        Dummy(ImVec2(text_size.x, text_size.y + style.FramePadding.y));
        g.NextItemData.ItemFlags |=
            ImGuiItemFlags_Inputable | ImGuiItemFlags_NoTabStop;
        EndChild();
        item_data_backup.StatusFlags |=
            (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_HoveredWindow);

        // ...and then we need to undo the group overriding last item data,
        // which gets a bit messy as EndGroup() tries to forward scrollbar being
        // active...
        // FIXME: This quite messy/tricky, should attempt to get rid of the
        // child window.
        EndGroup();
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(
        id, label, g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Inputable);
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state) {
    ...
    if (BeginChild("undopoints", ImVec2(0.0f, GetTextLineHeight() * 10),
                   ImGuiChildFlags_Border |
                       ImGuiChildFlags_ResizeY)) // Visualize undo state
    {
        ...
        for (int n = 0; n < IMSTB_TEXTEDIT_UNDOSTATECOUNT; n++) {
            ...
            if (undo_rec_type != ' ' && undo_rec->char_storage != -1)
                ImTextStrToUtf8(buf, IM_ARRAYSIZE(buf),
...
bool ImGui::ColorEdit4(const char* label, float col[4],
    ...
    IM_ASSERT(ImIsPowerOfTwo(
        flags &
        ImGuiColorEditFlags_DisplayMask_)); // Check that only 1 is selected
    IM_ASSERT(ImIsPowerOfTwo(
        flags &
        ImGuiColorEditFlags_InputMask_)); // Check that only 1 is selected
    ...
    if ((flags & ImGuiColorEditFlags_InputHSV) &&
        ...
        // Hue is lost when converting from grayscale rgb (saturation=0).
        // Restore it.
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB |
        ...
        const bool hide_prefix =
            (IM_TRUNC(w_items / components) <=
             CalcTextSize((flags & ImGuiColorEditFlags_Float) ? "M:0.000"
                                                              : "M:000")
                 .x);
        ...
        const int fmt_idx = hide_prefix                                ? 0
                            : (flags & ImGuiColorEditFlags_DisplayHSV) ? 2
                                                                       : 1;
        ...
        for (int n = 0; n < components; n++) {
            ...
            // FIXME: When ImGuiColorEditFlags_HDR flag is passed HS values snap
            // in weird ways when SV values go below 0.
            if (flags & ImGuiColorEditFlags_Float) {
                value_changed |=
                    DragFloat(ids[n], &f[n], 1.0f / 255.0f, 0.0f,
        ...
        char buf[64];
        if (alpha)
            ImFormatString(buf, IM_ARRAYSIZE(buf), "#%02X%02X%02X%02X",
        ...
        if (InputText("##Text", buf, IM_ARRAYSIZE(buf),
            ...
            if (alpha)
                r = sscanf(
                    p, "%02X%02X%02X%02X", (unsigned int*)&i[0],
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel)) {
        ...
        // style.ColorButtonPosition == ImGuiDir_Left), but we need to use
        // SameLine() to setup baseline correctly. Might want to refactor
        // SameLine() to simplify this.
        SameLine(0.0f, style.ItemInnerSpacing.x);
        window->DC.CursorPos.x =
            pos.x + ((flags & ImGuiColorEditFlags_NoInputs)
                         ? w_button
    ...
    if ((g.LastItemData.StatusFlags & ImGuiItemStatusFlags_HoveredRect) &&
        ...
        if (accepted_drag_drop && (flags & ImGuiColorEditFlags_InputHSV))
            ColorConvertRGBtoHSV(col[0], col[1], col[2], col[0], col[1],
                                 col[2]);
        EndDragDropTarget();
    }
    ...
    if (value_changed &&
        g.LastItemData.ID != 0) // In case of ID collision, the second
...
static void RenderArrowsForVerticalBar(ImDrawList* draw_list, ImVec2 pos,
    ...
    ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x + half_sz.x, pos.y),
                                 half_sz, ImGuiDir_Right,
                                 IM_COL32(255, 255, 255, alpha8));
    ImGui::RenderArrowPointingAt(draw_list,
                                 ImVec2(pos.x + bar_w - half_sz.x - 1, pos.y),
                                 ImVec2(half_sz.x + 2, half_sz.y + 1),
                                 ImGuiDir_Left, IM_COL32(0, 0, 0, alpha8));
...
bool ImGui::ColorPicker4(const char* label, float col[4],
    ...
    IM_ASSERT(ImIsPowerOfTwo(
        flags &
        ImGuiColorEditFlags_PickerMask_)); // Check that only 1 is selected
    IM_ASSERT(ImIsPowerOfTwo(
        flags &
        ImGuiColorEditFlags_InputMask_)); // Check that only 1 is selected
    if (!(flags & ImGuiColorEditFlags_NoOptions))
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel) {
        ...
        InvisibleButton("hsv", ImVec2(sv_picker_size +
                                          style.ItemInnerSpacing.x + bars_width,
                                      sv_picker_size));
        if (IsItemActive() && !is_readonly) {
            ...
            if (ImTriangleContainsPoint(
                ...
                if (!ImTriangleContainsPoint(triangle_pa, triangle_pb,
                                             triangle_pc,
                    ...
                    current_off_unrotated = ImTriangleClosestPoint(
                        ...
                        current_off_unrotated);
                float uu, vv, ww;
                ImTriangleBarycentricCoords(triangle_pa, triangle_pb,
                                            triangle_pc, current_off_unrotated,
                                            uu, vv, ww);
        ...
        if (IsItemActive() && !is_readonly) {
            ...
            ColorEditRestoreH(
                col,
                &H); // Greatly reduces hue jitter and reset to 0 when hue ==
                     // 255 and color is rapidly modified using SV square.
            value_changed = value_changed_sv = true;
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick("context", ImGuiPopupFlags_MouseButtonRight);
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0) {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB ||
            (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            if (ColorEdit4("##rgb", col,
                           sub_flags | ImGuiColorEditFlags_DisplayRGB)) {
                // FIXME: Hackily differentiating using the DragInt (ActiveId !=
                // 0 && !ActiveIdAllowOverlap) vs. using the InputText or
                // DropTarget. For the later we don't want to run the hue-wrap
                // canceling code. If you are well versed in HSV picker please
    ...
    if (value_changed_fix_hue_wrap && (flags & ImGuiColorEditFlags_InputRGB)) {
        ...
        if (new_H <= 0 && H > 0) {
            if (new_V <= 0 && V != new_V)
                ColorConvertHSVtoRGB(H, S, new_V <= 0 ? V * 0.5f : new_V,
                                     col[0], col[1], col[2]);
            else if (new_S <= 0)
    ...
    ImU32 user_col32_striped_of_alpha = ColorConvertFloat4ToU32(
        ImVec4(R, G, B, style.Alpha)); // Important: this is still including the
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel) {
        // Render Hue Wheel
        const float aeps = 0.5f / wheel_r_outer; // Half a pixel arc length in
                                                 // radians (2pi cancels out).
        ...
        for (int n = 0; n < 6; n++) {
            ...
            draw_list->PathArcTo(wheel_center,
                                 (wheel_r_inner + wheel_r_outer) * 0.5f, a0, a1,
                                 segment_per_arc);
            ...
            ShadeVertsLinearColorGradientKeepAlpha(
                draw_list, vert_start_idx, vert_end_idx, gradient_p0,
        ...
        ImVec2 tra =
            wheel_center + ImRotate(triangle_pa, cos_hue_angle, sin_hue_angle);
        ImVec2 trb =
            wheel_center + ImRotate(triangle_pb, cos_hue_angle, sin_hue_angle);
        ImVec2 trc =
            wheel_center + ImRotate(triangle_pc, cos_hue_angle, sin_hue_angle);
        ...
        sv_cursor_pos =
            ImLerp(ImLerp(trc, tra, ImSaturate(S)), trb, ImSaturate(1 - V));
    } else if (flags & ImGuiColorEditFlags_PickerHueBar) {
        // Render SV Square
        ...
        draw_list->AddRectFilledMultiColor(
            picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), 0,
            0, col_black, col_black);
        RenderFrameBorder(picker_pos,
                          picker_pos + ImVec2(sv_picker_size, sv_picker_size),
                          0.0f);
        sv_cursor_pos.x =
            ImClamp(IM_ROUND(picker_pos.x + ImSaturate(S) * sv_picker_size),
                    picker_pos.x + 2,
    ...
    float sv_cursor_rad =
        value_changed_sv ? wheel_thickness * 0.55f : wheel_thickness * 0.40f;
    ...
    if (alpha_bar) {
        ...
        RenderColorRectWithAlphaCheckerboard(
            draw_list, bar1_bb.Min, bar1_bb.Max, 0, bar1_bb.GetWidth() / 2.0f,
            ImVec2(0.0f, 0.0f));
        draw_list->AddRectFilledMultiColor(
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col,
                        ImGuiColorEditFlags flags, const ImVec2& size_arg) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
        off = -0.75f; // The border (using Col_FrameBg) tends to look off when
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f) {
        ...
        RenderColorRectWithAlphaCheckerboard(
            window->DrawList,
            ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max,
            GetColorU32(col_rgb), grid_step, ImVec2(-grid_step + off, off),
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(
                ...
                GetColorU32(col_source), grid_step, ImVec2(off, off), rounding);
        else
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
        if (g.Style.FrameBorderSize > 0.0f)
            ...
            window->DrawList->AddRect(bb.Min, bb.Max,
                                      ...
                                      rounding); // Color button are often in
                                                 // need of some sort of border
    }
    ...
    if (g.ActiveId == id && !(flags & ImGuiColorEditFlags_NoDragDrop) &&
        BeginDragDropSource()) {
        if (flags & ImGuiColorEditFlags_NoAlpha)
            SetDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_3F, &col_rgb,
                               sizeof(float) * 3, ImGuiCond_Once);
        else
            SetDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_4F, &col_rgb,
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags) {
    ...
    if ((flags & ImGuiColorEditFlags_DisplayMask_) == 0)
        flags |= ImGuiColorEditFlags_DefaultOptions_ &
                 ImGuiColorEditFlags_DisplayMask_;
    ...
    IM_ASSERT(ImIsPowerOfTwo(
        flags &
        ImGuiColorEditFlags_DisplayMask_)); // Check only 1 option is selected
    IM_ASSERT(ImIsPowerOfTwo(
        flags &
        ImGuiColorEditFlags_DataTypeMask_)); // Check only 1 option is selected
    IM_ASSERT(ImIsPowerOfTwo(
        flags &
        ImGuiColorEditFlags_PickerMask_)); // Check only 1 option is selected
    IM_ASSERT(ImIsPowerOfTwo(
        flags &
        ImGuiColorEditFlags_InputMask_)); // Check only 1 option is selected
...
void ImGui::ColorTooltip(const char* text, const float* col,
    ...
    int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]),
        ...
        ca = (flags & ImGuiColorEditFlags_NoAlpha) ? 255
    ...
    if ((flags & ImGuiColorEditFlags_InputRGB) ||
        ...
        if (flags & ImGuiColorEditFlags_NoAlpha)
            Text("#%02X%02X%02X\nR: %d, G: %d, B: %d\n(%.3f, %.3f, %.3f)", cr,
                 cg, cb, cr, cg, cb, col[0], col[1], col[2]);
        else
            Text("#%02X%02X%02X%02X\nR:%d, G:%d, B:%d, A:%d\n(%.3f, %.3f, "
                 "%.3f, %.3f)",
                 cr, cg, cb, ca, cr, cg, cb, ca, col[0], col[1], col[2],
                 col[3]);
    } else if (flags & ImGuiColorEditFlags_InputHSV) {
        if (flags & ImGuiColorEditFlags_NoAlpha)
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags) {
    ...
    if (allow_opt_inputs) {
        if (RadioButton("RGB", (opts & ImGuiColorEditFlags_DisplayRGB) != 0))
            opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) |
    ...
    if (allow_opt_datatype) {
        ...
        if (RadioButton("0..255", (opts & ImGuiColorEditFlags_Uint8) != 0))
            opts = (opts & ~ImGuiColorEditFlags_DataTypeMask_) |
    ...
    if (BeginPopup("Copy")) {
        int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]),
            ...
            ca = (flags & ImGuiColorEditFlags_NoAlpha)
                     ...
                     : IM_F32_TO_INT8_SAT(col[3]);
        char buf[64];
        ImFormatString(buf, IM_ARRAYSIZE(buf), "(%.3ff, %.3ff, %.3ff, %.3ff)",
                       col[0], col[1], col[2],
                       (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
        if (Selectable(buf))
        ...
        if (Selectable(buf))
            SetClipboardText(buf);
        if (!(flags & ImGuiColorEditFlags_NoAlpha)) {
            ImFormatString(buf, IM_ARRAYSIZE(buf), "#%02X%02X%02X%02X", cr, cg,
                           cb, ca);
            if (Selectable(buf))
                SetClipboardText(buf);
        }
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col,
    ...
    if (allow_opt_picker) {
        ...
        for (int picker_type = 0; picker_type < 2; picker_type++) {
            // Draw small/thumbnail version of each picker type (over an
            ...
            memcpy(&previewing_ref_col, ref_col,
                   sizeof(float) *
                       ((picker_flags & ImGuiColorEditFlags_NoAlpha) ? 3 : 4));
            ColorPicker4("##previewing_picker", &previewing_ref_col.x,
                         picker_flags);
            PopID();
        }
        PopItemWidth();
...
bool ImGui::TreeNodeUpdateNextOpen(ImGuiID storage_id,
    ...
    // We only write to the tree storage if the user clicks, or explicitly use
    // the SetNextItemOpen function
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    if (g.NextItemData.Flags & ImGuiNextItemDataFlags_HasOpen) {
        if (g.NextItemData.OpenCond & ImGuiCond_Always) {
            ...
            // We treat ImGuiCond_Once and ImGuiCond_FirstUseEver the same
            // because tree node state are not saved persistently.
        ...
        is_open = storage->GetInt(
                      storage_id,
                      (flags & ImGuiTreeNodeFlags_DefaultOpen) ? 1 : 0) != 0;
    }

    // When logging is enabled, we automatically expand tree nodes (but *NOT*
    // collapsing headers.. seems like sensible behavior). NB- If we are above
    // max depth we still allow manually opened nodes to be logged.
...
// Currently only supports 32 level deep and we are fine with (1 << Depth)
// overflowing into a zero, easy to increase.
...
// When using public API, currently 'id == storage_id' is always true, but we
// separate the values to facilitate advanced user code doing storage queries
// outside of UI loop.
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
    ...
    const float text_width =
        g.FontSize + label_size.x + padding.x * 2; // Include collapsing arrow
    ...
    if ((flags &
        ...
        interact_bb.Max.x =
            frame_bb.Min.x + text_width +
            (label_size.x > 0.0f ? style.ItemSpacing.x * 2.0f : 0.0f);

    // Compute open and multi-select states before ItemAdd() as it clear
    // NextItem data.
    ImGuiID storage_id =
    ...
    // ImGuiTreeNodeFlags_NavLeftJumpsBackHere enabled: Store data for the
    // current depth to allow returning to this node from any child item. For
    // this purpose we essentially compare if g.NavIdIsAlive went from 0 to 1
    // between TreeNode() and TreePop(). It will become tempting to enable
    // ImGuiTreeNodeFlags_NavLeftJumpsBackHere by default or move it to
    // ImGuiStyle.
    bool store_tree_node_stack_data = false;
    if (!(flags & ImGuiTreeNodeFlags_NoTreePushOnOpen)) {
        if ((flags & ImGuiTreeNodeFlags_NavLeftJumpsBackHere) && is_open &&
            !g.NavIdIsAlive)
    ...
    // order to easily allow browsing a tree while preserving selection with
    // code implementing multi-selection patterns. When clicking on the rest of
    // the tree node we always disallow keyboard modifiers.
    ...
    const bool is_multi_select =
        (g.LastItemData.InFlags & ImGuiItemFlags_IsMultiSelect) != 0;
    if (is_multi_select) // We absolutely need to distinguish open vs select so
                         // _OpenOnArrow comes by default
    ...
    // MouseDown events) due to requirements for multi-selection and drag and
    // drop support.
    ...
    // ImGuiButtonFlags_PressedOnClickRelease on OpenOnDoubleClick because we
    // want the item to be active on the initial MouseDown in order for drag and
    // drop to work.
    ...
    if (!is_leaf) {
        if (pressed && g.DragDropHoldJustPressedId != id) {
            ...
            if (!is_open) // When using Drag and Drop "hold to open" we keep the
                          // node highlighted after opening, but never close it
                          // again.
                toggled = true;
            else
                pressed =
                    false; // Cancel press so it doesn't trigger selection.
        }
        ...
        if (g.NavId == id && g.NavMoveDir == ImGuiDir_Right &&
            !is_open) // If there's something upcoming on the line we may want
                      // to give it the priority?
        {
            toggled = true;
            NavClearPreferredPosForAxis(ImGuiAxis_X);
            NavMoveRequestCancel();
    ...
    // Render
    {
        ...
        if (is_multi_select)
            nav_highlight_flags |=
                ImGuiNavHighlightFlags_AlwaysDraw; // Always show the nav
                                                   // rectangle
        if (display_frame) {
            // Framed type
            const ImU32 bg_col =
    ...
    if (is_open && !(flags & ImGuiTreeNodeFlags_NoTreePushOnOpen))
        TreePushOverrideID(
            id); // Could use TreePush(label) but this avoid computing twice
...
void ImGui::TreePop() {
        ...
        tree_depth_mask) // Only set during request
    {
        ...
        if (data->TreeFlags & ImGuiTreeNodeFlags_NavLeftJumpsBackHere) {
            ...
            if (g.NavIdIsAlive && g.NavMoveDir == ImGuiDir_Left &&
...
bool ImGui::CollapsingHeader(const char* label, ImGuiTreeNodeFlags flags) {
    ...
    if (window->SkipItems)
        return false;
    ImGuiID id = window->GetID(label);
    return TreeNodeBehavior(id, flags | ImGuiTreeNodeFlags_CollapsingHeader,
                            label);
}
...
// adjust with SetNextItemOpen() or ImGuiTreeNodeFlags_DefaultOpen.
bool ImGui::CollapsingHeader(const char* label, bool* p_visible,
                             ImGuiTreeNodeFlags flags) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
// ImGuiSelectableFlags_SpanAllColumns and ImGuiSelectableFlags_AllowOverlap are
...
bool ImGui::Selectable(const char* label, bool selected,
    ...
    // FIXME: Not part of layout so not included in clipper calculation, but
    // ItemSize currently doesn't allow offsetting CursorPos.
    ...
    const ImGuiItemFlags extra_item_flags =
        disabled_item ? (ImGuiItemFlags)ImGuiItemFlags_Disabled
                      : ImGuiItemFlags_None;
    bool is_visible;
    if (span_all_columns) {
        // Modify ClipRect for the ItemAdd(), faster than doing a
    ...
    if (!is_visible)
        if (!is_multi_select || !g.BoxSelectState.UnclipMode ||
            !g.BoxSelectState.UnclipRect.Overlaps(
                bb)) // Extra layer of "no logic clip" for box-select support
                     // (would be more overhead to add to ItemAdd)
            return false;
    ...
    if (is_multi_select) {
        ...
        if ((flags & ImGuiSelectableFlags_SelectOnNav) &&
    ...
    if (is_visible) {
        ...
        if (highlighted || selected) {
            // FIXME-MULTISELECT: Styling: Color for 'selected' elements?
            // ImGuiCol_HeaderSelected
            ImU32 col;
            if (selected && !highlighted)
                col = GetColorU32(
                    ImLerp(GetStyleColorVec4(ImGuiCol_Header),
                           GetStyleColorVec4(ImGuiCol_HeaderHovered), 0.5f));
            else
                col = GetColorU32((held && highlighted) ? ImGuiCol_HeaderActive
    ...
    // ImGui::IsItemToggledSelection() to retrieve selection toggle, only useful
    // if you need that state updated (e.g. for rendering purpose) before
...
//           focus_idx = ImGui::TypingSelectFindMatch(req, my_items.size(),
...
ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags) {
    ImGuiContext& g = *GImGui;
    ...
    if (data->SearchBuffer[0] != 0) {
        ...
        clear_buffer |= IsKeyPressed(ImGuiKey_Backspace) &&
                        (flags & ImGuiTypingSelectFlags_AllowBackspace) == 0;
        // if (clear_buffer) { IMGUI_DEBUG_LOG("GetTypingSelectRequest(): Clear
        // SearchBuffer.\n"); }
        if (clear_buffer)
            data->Clear();
    }
    ...
    for (ImWchar w : g.IO.InputQueueCharacters) {
        ...
        if (data->SingleCharModeLock && w_len == out_request->SingleCharSize &&
            ...
            select_request =
                true; // Same character: don't need to append to buffer.
            continue;
        }
    ...
    if ((flags & ImGuiTypingSelectFlags_AllowBackspace) &&
        ...
        char* p = (char*)(void*)ImTextFindPreviousUtf8Codepoint(
...
//   if your SetNextItemSelectionUserData() values are indices, you can obtain
//   it from ImGuiMultiSelectIO::NavIdItem, otherwise from
...
int ImGui::TypingSelectFindMatch(ImGuiTypingSelectRequest* req, int items_count,
    ...
    if (req->SingleCharMode &&
        (req->Flags & ImGuiTypingSelectFlags_AllowSingleCharMode))
        idx = TypingSelectFindNextSingleCharMatch(
            req, items_count, get_item_name_func, user_data, nav_item_idx);
    else
        idx = TypingSelectFindBestLeadingMatch(req, items_count,
...
int ImGui::TypingSelectFindNextSingleCharMatch(
    ImGuiTypingSelectRequest* req, int items_count,
    ...
    for (int idx = 0; idx < items_count; idx++) {
        ...
        if (ImStrimatchlen(req->SearchBuffer,
        ...
        if (first_match_idx == -1 &&
            nav_item_idx == -1) // Return first match immediately if we don't
                                // have a nav_item_idx value.
            return idx;
        if (first_match_idx == -1) // Record first match for wrapping.
            first_match_idx = idx;
        if (nav_item_idx == idx) // Record that we encountering nav_item so we
                                 // can return next match.
...
int ImGui::TypingSelectFindBestLeadingMatch(
    ...
    for (int idx = 0; idx < items_count; idx++) {
        const char* item_name = get_item_name_func(user_data, idx);
        const int match_len =
            ImStrimatchlen(req->SearchBuffer,
                           req->SearchBuffer + req->SearchBufferLen, item_name);
...
//-------------------------------------------------------------------------
// [SECTION] Widgets: Box-Select support
// This has been extracted away from Multi-Select logic in the hope that it
// could eventually be used elsewhere, but hasn't been yet.
//-------------------------------------------------------------------------
...
static void BoxSelectPreStartDrag(ImGuiID id,
                                  ImGuiSelectionUserData clicked_item) {
    ImGuiContext& g = *GImGui;
    ...
    bs->StartPosRel = bs->EndPosRel =
        ImGui::WindowPosAbsToRel(g.CurrentWindow, g.IO.MousePos);
    bs->ScrollAccum = ImVec2(0.0f, 0.0f);
}

static void BoxSelectActivateDrag(ImGuiBoxSelectState* bs,
                                  ImGuiWindow* window) {
    ImGuiContext& g = *GImGui;
    ...
    if (bs->IsStartedFromVoid &&
        (bs->KeyMods & (ImGuiMod_Ctrl | ImGuiMod_Shift)) == 0)
...
static void BoxSelectScrollWithMouseDrag(ImGuiBoxSelectState* bs,
                                         ImGuiWindow* window,
    ...
    for (int n = 0; n < 2; n++) // each axis
    {
        ...
        const float dist =
            (mouse_pos > inner_r.Max[n])   ? mouse_pos - inner_r.Max[n]
...
bool ImGui::BeginBoxSelect(const ImRect& scope_rect, ImGuiWindow* window,
    ...
    // IsStarting is set by MultiSelectItemFooter() when considering a possible
    // box-select. We validate it here and lock geometry.
    bs->UnclipMode = false;
    ...
    // Current frame absolute prev/current rectangles are used to toggle
    // selection. They are derived from positions relative to scrolling space.
    ImVec2 start_pos_abs = WindowPosRelToAbs(window, bs->StartPosRel);
    ImVec2 prev_end_pos_abs =
        WindowPosRelToAbs(window, bs->EndPosRel); // Clamped already
    ImVec2 curr_end_pos_abs = g.IO.MousePos;
    if (ms_flags &
        ImGuiMultiSelectFlags_ScopeWindow) // Box-select scrolling only happens
                                           // with ScopeWindow
        curr_end_pos_abs =
            ImClamp(curr_end_pos_abs, scope_rect.Min, scope_rect.Max);
    ...
    // Box-select 2D mode detects horizontal changes (vertical ones are already
    // picked by Clipper) Storing an extra rect used by widgets supporting
    // box-select.
    ...
    // IM_COL32(255,0,0,200), 0.0f, 0, 3.0f);
    // GetForegroundDrawList()->AddRect(bs->BoxSelectRectPrev.Min,
    // bs->BoxSelectRectPrev.Max, IM_COL32(255,0,0,200), 0.0f, 0, 3.0f);
    // GetForegroundDrawList()->AddRect(bs->BoxSelectRectCurr.Min,
    // bs->BoxSelectRectCurr.Max, IM_COL32(0,255,0,200), 0.0f, 0, 1.0f);
    return true;
}
...
void ImGui::EndBoxSelect(const ImRect& scope_rect,
    ...
    bs->EndPosRel = WindowPosAbsToRel(
        window, ImClamp(g.IO.MousePos, scope_rect.Min,
                        scope_rect.Max)); // Clamp stored position according to
                                          // current scrolling view
    ImRect box_select_r = bs->BoxSelectRectCurr;
    box_select_r.ClipWith(scope_rect);
    window->DrawList->AddRectFilled(
        box_select_r.Min, box_select_r.Max,
        GetColorU32(ImGuiCol_SeparatorHovered,
                    0.30f)); // FIXME-MULTISELECT: Styling
    window->DrawList->AddRect(
        box_select_r.Min, box_select_r.Max,
        GetColorU32(ImGuiCol_NavHighlight)); // FIXME-MULTISELECT: Styling

    // Scroll
    const bool enable_scroll =
        (ms_flags & ImGuiMultiSelectFlags_ScopeWindow) &&
        (ms_flags & ImGuiMultiSelectFlags_BoxSelectNoScroll) == 0;
    if (enable_scroll) {
        ImRect scroll_r = scope_rect;
        scroll_r.Expand(-g.FontSize);
        // GetForegroundDrawList()->AddRect(scroll_r.Min, scroll_r.Max,
        // IM_COL32(0, 255, 0, 255));
...
static void DebugLogMultiSelectRequests(const char* function,
    ...
    for (const ImGuiSelectionRequest& req : io->Requests) {
        ...
        if (req.Type == ImGuiSelectionRequestType_SetRange)
            IMGUI_DEBUG_LOG_SELECTION(
                "[selection] %s: Request: SetRange %" IM_PRId64 "..%" IM_PRId64
                " (0x%" IM_PRIX64 "..0x%" IM_PRIX64 ") = %d (dir %d)\n",
                function, req.RangeFirstItem, req.RangeLastItem,
                req.RangeFirstItem, req.RangeLastItem, req.Selected,
                req.RangeDirection);
    }
}
...
static ImRect CalcScopeRect(ImGuiMultiSelectTempData* ms, ImGuiWindow* window) {
    if (ms->Flags & ImGuiMultiSelectFlags_ScopeRect) {
        // Warning: this depends on CursorMaxPos so it means to be called by
        // EndMultiSelect() only
        return ImRect(ms->ScopeRectMin,
                      ImMax(window->DC.CursorMaxPos, ms->ScopeRectMin));
    } else {
        // Add inner table decoration (#7821) // FIXME: Why not baking in
        // InnerClipRect?
        ImRect scope_rect = window->InnerClipRect;
        scope_rect.Min = ImMin(scope_rect.Min + ImVec2(window->DecoInnerSizeX1,
...
// Lifetime: don't hold on ImGuiMultiSelectIO* pointers over multiple frames or
// past any subsequent call to BeginMultiSelect() or EndMultiSelect(). Passing
// 'selection_size' and 'items_count' parameters is currently optional.
// - 'selection_size' is useful to disable some shortcut routing: e.g.
// ImGuiMultiSelectFlags_ClearOnEscape won't claim Escape key when
// selection_size 0,
//    allowing a first press to clear selection THEN the second press to leave
//    child window and return to parent.
// - 'items_count' is stored in ImGuiMultiSelectIO which makes it a convenient
// way to pass the information to your ApplyRequest() handler (but you may pass
// it differently).
// - If they are costly for you to compute (e.g. external intrusive selection
// without maintaining size), you may avoid them and pass -1.
//   - If you can easily tell if your selection is empty or not, you may pass
//   0/1, or you may enable ImGuiMultiSelectFlags_ClearOnEscape flag
//   dynamically.
ImGuiMultiSelectIO* ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags,
    ...
    IM_STATIC_ASSERT(offsetof(ImGuiMultiSelectTempData, IO) ==
                     0); // Clear() relies on that.
    g.CurrentMultiSelect = ms;
    if ((flags & (ImGuiMultiSelectFlags_ScopeWindow |
                  ImGuiMultiSelectFlags_ScopeRect)) == 0)
    ...
    if (flags &
        ImGuiMultiSelectFlags_ScopeWindow) // Mark parent child window as
                                           // navigable into, with highlight.
                                           // Assume user will always submit
                                           // interactive items.
        window->DC.NavLayersActiveMask |= 1 << ImGuiNavLayer_Main;

    // Use copy of keyboard mods at the time of the request, otherwise we would
    // requires mods to be held for an extra frame.
    ms->KeyMods =
        g.NavJustMovedToId
            ? (g.NavJustMovedToIsTabbing ? 0 : g.NavJustMovedToKeyMods)
            : g.IO.KeyMods;
    ...
    // (we compare FocusScopeId so it possible to use multiple selections inside
    // a same window)
    ...
    if (g.NavJustMovedToId != 0 &&
        ...
        if ((ms->KeyMods & (ImGuiMod_Ctrl | ImGuiMod_Shift)) == 0 &&
            (flags & (ImGuiMultiSelectFlags_NoAutoClear |
                      ImGuiMultiSelectFlags_NoAutoSelect)) == 0)
            request_clear = true;
    ...
    if (flags & (ImGuiMultiSelectFlags_BoxSelect1d |
        ...
        if (BeginBoxSelect(CalcScopeRect(ms, window), window, ms->BoxSelectId,
                           flags))
    ...
    if (ms->IsFocused) {
        ...
        // - Only claim shortcut if selection is not empty, allowing further
        // presses on Escape to e.g. leave current child window.
        ...
        if (flags & ImGuiMultiSelectFlags_ClearOnEscape) {
            if (selection_size != 0 || bs->IsActive)
                if (Shortcut(ImGuiKey_Escape, ImGuiInputFlags_None,
                             bs->IsActive ? bs->ID : 0)) {
                    request_clear = true;
                    if (bs->IsActive)
                        BoxSelectDeactivateDrag(bs);
...
// Lifetime: don't hold on ImGuiMultiSelectIO* pointers over multiple frames or
// past any subsequent call to BeginMultiSelect() or EndMultiSelect().
ImGuiMultiSelectIO* ImGui::EndMultiSelect() {
    ...
    if (ms->IsFocused) {
        // We currently don't allow user code to modify RangeSrcItem by writing
        // to BeginIO's version, but that would be an easy change here.
        if (ms->IO.RangeSrcReset ||
            (ms->RangeSrcPassedBy == false &&
             ms->IO.RangeSrcItem !=
                 ImGuiSelectionUserData_Invalid)) // Can't read
                                                  // storage->RangeSrcItem here
                                                  // -> we want the state at
                                                  // begining of the scope (see
                                                  // tests for easy failure)
        {
            IMGUI_DEBUG_LOG_SELECTION(
        ...
        if ((ms->Flags & (ImGuiMultiSelectFlags_BoxSelect1d |
                          ImGuiMultiSelectFlags_BoxSelect2d)) &&
    ...
    // We specifically test for IsMouseDragPastThreshold(0) == false to allow
    // box-selection! The InnerRect test is necessary for non-child/decorated
    // windows.
    bool scope_hovered =
        IsWindowHovered() && window->InnerRect.Contains(g.IO.MousePos);
    ...
    if (scope_hovered && g.HoveredId == 0 && g.ActiveId == 0) {
        if (ms->Flags & (ImGuiMultiSelectFlags_BoxSelect1d |
            ...
            if (!g.BoxSelectState.IsActive && !g.BoxSelectState.IsStarting &&
                ...
                if (ms->Flags & ImGuiMultiSelectFlags_ScopeRect)
                    SetNavID(
                        0, ImGuiNavLayer_Main, ms->FocusScopeId,
                        ImRect(g.IO.MousePos,
                               g.IO.MousePos)); // Automatically switch
                                                // FocusScope for initial click
    ...
    if (ms->Flags & ImGuiMultiSelectFlags_NavWrapX) {
        IM_ASSERT(ms->Flags &
                  ImGuiMultiSelectFlags_ScopeWindow); // Only supported at
                                                      // window scope
    ...
    // Unwind
    window->DC.CursorMaxPos =
        ImMax(ms->BackupCursorMaxPos, window->DC.CursorMaxPos);
    PopFocusScope();
...
void ImGui::SetNextItemSelectionUserData(
    ImGuiSelectionUserData selection_user_data) {
    // Note that flags will be cleared by ItemAdd(), so it's only useful for
    // Navigation code! This designed so widgets can also cheaply set this
    // before calling ItemAdd(), so we are not tied to MultiSelect api.
    ImGuiContext& g = *GImGui;
    ...
    if (ImGuiMultiSelectTempData* ms = g.CurrentMultiSelect) {
        ...
        // (done before ItemAdd() clipping)
        g.NextItemData.ItemFlags |=
            ImGuiItemFlags_HasSelectionUserData | ImGuiItemFlags_IsMultiSelect;
...
void ImGui::MultiSelectItemHeader(ImGuiID id, bool* p_selected,
    ...
    if (ms->IsFocused) {
        ...
        IM_ASSERT(
            g.NextItemData.FocusScopeId == g.CurrentFocusScopeId &&
            "Forgot to call SetNextItemSelectionUserData() prior to item, "
            "required in BeginMultiSelect()/EndMultiSelect() scope");

        // Apply SetAll (Clear/SelectAll) requests requested by
        // BeginMultiSelect(). This is only useful if the user hasn't processed
        // them already, and this only works if the user isn't using the
        // clipper. If you are using a clipper you need to process the SetAll
        // request after calling BeginMultiSelect()
        if (ms->LoopRequestSetAll != -1)
            selected = (ms->LoopRequestSetAll == 1);

        // When using SHIFT+Nav: because it can incur scrolling we cannot afford
        // a frame of lag with the selection highlight (otherwise scrolling
        // would happen before selection) For this to work, we need someone to
        // set 'RangeSrcPassedBy = true' at some point (either clipper either
        ...
        if (ms->IsKeyboardSetRange) {
            ...
            const bool is_range_dst =
                ...
                g.NavJustMovedToId ==
                    id; // Assume that g.NavJustMovedToId is not clipped.
            if (is_range_dst)
                ms->RangeDstPassedBy = true;
            if (is_range_dst &&
                storage->RangeSrcItem ==
                    ImGuiSelectionUserData_Invalid) // If we don't have
                                                    // RangeSrc, assign RangeSrc
                                                    // = RangeDst
            {
    ...
    // selection on click. Enabling this test makes actions using CTRL+SHIFT
    // delay their effect on MouseUp which is annoying, but it allows drag and
    // drop of multiple items.
    if (p_button_flags != NULL) {
        ImGuiButtonFlags button_flags = *p_button_flags;
        button_flags |= ImGuiButtonFlags_NoHoveredOnFocus;
        if ((!selected ||
             (g.ActiveId == id && g.ActiveIdHasBeenPressedBefore)) &&
...
// - Altering selection based on Ctrl/Shift modifiers, both for keyboard and
// mouse.
// - Record current selection state for RangeSrc
// This is all rather complex, best to run and refer to
// "widgets_multiselect_xxx" tests in imgui_test_suite.
void ImGui::MultiSelectItemFooter(ImGuiID id, bool* p_selected,
                                  bool* p_pressed) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    const bool is_singleselect =
        (flags & ImGuiMultiSelectFlags_SingleSelect) != 0;
    ...
    if (g.NavJustMovedToId == id) {
        if ((flags & ImGuiMultiSelectFlags_NoAutoSelect) == 0) {
            ...
            if (is_shift)
                ...
                apply_to_range_src = true; // Since if (pressed) {} main block
                                           // is not running we update this
        }
    }
    ...
    if (ms->BoxSelectId != 0)
        if (ImGuiBoxSelectState* bs = GetBoxSelectState(ms->BoxSelectId)) {
            const bool rect_overlap_curr =
                bs->BoxSelectRectCurr.Overlaps(g.LastItemData.Rect);
            const bool rect_overlap_prev =
                bs->BoxSelectRectPrev.Overlaps(g.LastItemData.Rect);
            if ((rect_overlap_curr && !rect_overlap_prev && !selected) ||
                ...
                if (storage->LastSelectionSize <= 0 &&
                    ...
                    selected = !selected;
                    MultiSelectAddSetRange(ms, selected, +1, item_data,
                                           item_data);
                }
                storage->LastSelectionSize =
                    ImMax(storage->LastSelectionSize + 1, 1);
            }
        }

    // Right-click handling.
    // FIXME-MULTISELECT: Currently filtered out by
    // ImGuiMultiSelectFlags_NoAutoSelect but maybe should be moved to
    // Selectable(). See https://github.com/ocornut/imgui/pull/5816
    if (hovered && IsMouseClicked(1) &&
        (flags & ImGuiMultiSelectFlags_NoAutoSelect) == 0) {
    ...
    // Unlike Space, Enter doesn't alter selection (but can still return a
    // press) unless current item is not selected. The later, "unless current
    // item is not select", may become optional? It seems like a better default
    // if Enter doesn't necessarily open something (unlike e.g. Windows
    // explorer). For use case where Enter always open something, we might
    // decide to make this optional?
    const bool enter_pressed =
        pressed && (g.NavActivateId == id) &&
        (g.NavActivateFlags & ImGuiActivateFlags_PreferInput);
    ...
    if (pressed && (!enter_pressed || !selected)) {
        ...
        ImGuiInputSource input_source =
            (g.NavJustMovedToId == id || g.NavActivateId == id)
                ? g.NavInputSource
        ...
        if (flags & (ImGuiMultiSelectFlags_BoxSelect1d |
            ...
            if (selected == false && !g.BoxSelectState.IsActive &&
                ...
                input_source == ImGuiInputSource_Mouse &&
                g.IO.MouseClickedCount[0] == 1)
        ...
        // Keys Navigated:      Shift  | n/a    | Dst=item, Sel=1,   => Clear +
        // SetRange 1 Keys Navigated: Ctrl+Shift  | n/a    | Dst=item, Sel=Src
        // => Clear + SetRange Src-Dst Keys Activated:             | n/a    |
        // Src=item, Sel=1    => Clear + SetRange 1 Keys Activated: Ctrl | n/a
        // | Src=item, Sel=!Sel =>         SetSange 1 Keys Activated:      Shift
        // | n/a    | Dst=item, Sel=1    => Clear + SetSange 1
        //----------------------------------------------------------------------------------------
        // Mouse Pressed:              | n/a    | Src=item, Sel=1,   => Clear +
        // SetRange 1 Mouse Pressed:  Ctrl        | n/a    | Src=item, Sel=!Sel
        // =>         SetRange 1 Mouse Pressed:       Shift  | n/a    |
        // Dst=item, Sel=1,   => Clear + SetRange 1 Mouse Pressed:  Ctrl+Shift
        // | n/a    | Dst=item, Sel=!Sel =>         SetRange Src-Dst
        //----------------------------------------------------------------------------------------
        ...
        if ((flags & ImGuiMultiSelectFlags_NoAutoClear) == 0) {
            ...
            if (is_singleselect)
                request_clear = true;
            else if ((input_source == ImGuiInputSource_Mouse ||
                      g.NavActivateId == id) &&
                     !is_ctrl)
                request_clear =
                    (flags & ImGuiMultiSelectFlags_NoAutoClearOnReselect)
                        ? !selected
                        : true;
                     ...
                     is_shift && !is_ctrl)
                request_clear =
                    true; // With is_shift==false the RequestClear was done in
                          // BeginIO, not necessary to do again.
            if (request_clear)
                MultiSelectAddSetAll(ms, false);
        }
        ...
        if (is_shift && !is_singleselect) {
            ...
            if ((flags & ImGuiMultiSelectFlags_NoAutoSelect) == 0) {
                ...
                // Ctrl+Shift+Arrow copy source selection state (already stored
                // by BeginMultiSelect() in storage->RangeSelected)
        ...
        MultiSelectAddSetRange(ms, range_selected, range_direction,
                               storage->RangeSrcItem, item_data);
    }

    // Update/store the selection state of the Source item (used by CTRL+SHIFT,
    // when Source is unselected we perform a range unselect)
...
void ImGui::MultiSelectAddSetRange(ImGuiMultiSelectTempData* ms, bool selected,
    ...
    // Merge contiguous spans into same request (unless NoRangeSelect is set
    // which guarantees single-item ranges)
    if (ms->IO.Requests.Size > 0 && first_item == last_item &&
        (ms->Flags & ImGuiMultiSelectFlags_NoRangeSelect) == 0) {
        ImGuiSelectionRequest* prev =
...
void ImGui::DebugNodeMultiSelectState(ImGuiMultiSelectState* storage) {
    ...
    bool open =
        TreeNode((void*)(intptr_t)storage->ID, "MultiSelect 0x%08X in '%s'%s",
                 storage->ID, storage->Window ? storage->Window->Name : "N/A",
                 is_active ? "" : " *Inactive*");
    if (!is_active) {
        PopStyleColor();
    }
    ...
    Text("NavIdItem = %" IM_PRId64 " (0x%" IM_PRIX64 "), NavIdSelected = %d",
         storage->NavIdItem, storage->NavIdItem, storage->NavIdSelected);
    Text("LastSelectionSize = %d",
         storage->LastSelectionSize); // Provided by user
    TreePop();
#else
    IM_UNUSED(storage);
#endif
...
ImGuiSelectionBasicStorage::ImGuiSelectionBasicStorage() {
    ...
    PreserveOrder = false;
    UserData = NULL;
    AdapterIndexToStorageId = [](ImGuiSelectionBasicStorage*, int idx) {
        return (ImGuiID)idx;
    };
...
static int IMGUI_CDECL PairComparerByValueInt(const void* lhs,
                                              const void* rhs) {
...
// GetNextSelectedItem() is an abstraction allowing us to change our underlying
// actual storage system without impacting user. (e.g. store unselected vs
// compact down, compact down on demand, use raw ImVector<ImGuiID> instead of
// ImGuiStorage...)
bool ImGuiSelectionBasicStorage::GetNextSelectedItem(void** opaque_it,
    ...
    if (PreserveOrder && it == NULL && it_end != NULL)
        ImQsort(_Storage.Data.Data, (size_t)_Storage.Data.Size,
                sizeof(ImGuiStoragePair),
...
static void ImGuiSelectionBasicStorage_BatchSetItemSelected(
    ImGuiSelectionBasicStorage* selection, ImGuiID id, bool selected,
    int size_before_amends, int selection_order) {
    ImGuiStorage* storage = &selection->_Storage;
    ImGuiStoragePair* it = ImLowerBound(
        storage->Data.Data, storage->Data.Data + size_before_amends, id);
    const bool is_contained =
        (it != storage->Data.Data + size_before_amends) && (it->key == id);
    if (selected == (is_contained && it->val_i != 0))
        return;
    if (selected && !is_contained)
        storage->Data.push_back(ImGuiStoragePair(
            id, selection_order)); // Push unsorted at end of vector, will be
...
static void
ImGuiSelectionBasicStorage_BatchFinish(ImGuiSelectionBasicStorage* selection,
...
// - Enable 'Demo->Tools->Debug Log->Selection' to see selection requests as
// they happen.
// - Honoring SetRange requests requires that you can iterate/interpolate
// between RangeFirstItem and RangeLastItem.
//   - In this demo we often submit indices to SetNextItemSelectionUserData() +
//   store the same indices in persistent selection.
//   - Your code may do differently. If you store pointers or objects ID in
//   ImGuiSelectionUserData you may need to perform
//     a lookup in order to have some way to iterate/interpolate between two
//     items.
// - A full-featured application is likely to allow search/filtering which is
// likely to lead to using indices
//   and constructing a view index <> object id/ptr data structure anyway.
// WHEN YOUR APPLICATION SETTLES ON A CHOICE, YOU WILL PROBABLY PREFER TO GET
// RID OF THIS UNNECESSARY 'ImGuiSelectionBasicStorage' INDIRECTION LOGIC.
// Notice that with the simplest adapter (using indices everywhere), all
// functions return their parameters. The most simple implementation (using
// indices everywhere) would look like:
//   for (ImGuiSelectionRequest& req : ms_io->Requests)
//   {
//      if (req.Type == ImGuiSelectionRequestType_SetAll)    { Clear(); if
//      (req.Selected) { for (int n = 0; n < items_count; n++) {
//      SetItemSelected(n, true); } } if (req.Type ==
//      ImGuiSelectionRequestType_SetRange)  { for (int n =
//      (int)ms_io->RangeFirstItem; n <= (int)ms_io->RangeLastItem; n++) {
//      SetItemSelected(n, ms_io->Selected); } }
//   }
void ImGuiSelectionBasicStorage::ApplyRequests(ImGuiMultiSelectIO* ms_io) {
    // For convenience we obtain ItemsCount as passed to BeginMultiSelect(),
    // which is optional. It makes sense when using ImGuiSelectionBasicStorage
    // to simply pass your items count to BeginMultiSelect(). Other scheme may
    // handle SetAll differently.
    IM_ASSERT(ms_io->ItemsCount != -1 &&
              "Missing value for items_count in BeginMultiSelect() call!");
    IM_ASSERT(AdapterIndexToStorageId != NULL);

    // This is optimized/specialized to cope with very large selections (e.g.
    // 100k+ items)
    ...
    // - Optimized select can append unsorted, then sort in a second pass.
    // Optimized unselect can clear in-place then compact in a second pass.
    // - A more optimal version wouldn't even use ImGuiStorage but directly a
    // ImVector<ImGuiID> to reduce bandwidth, but this is a reasonable trade off
    // to reuse code.
    // - There are many ways this could be better optimized. The worse case
    // scenario being: using BoxSelect2d in a grid, box-select scrolling down
    // while wiggling
    //   left and right: it affects coarse clipping + can emit multiple SetRange
    //   with 1 item each.)
    // FIXME-OPT: For each block of consecutive SetRange request:
    // - add all requests to a sorted list, store ID, selected, offset in
    // ImGuiStorage.
        ...
        if (req.Type == ImGuiSelectionRequestType_SetAll) {
            ...
            if (req.Selected) {
                ...
                for (int idx = 0; idx < ms_io->ItemsCount;
                     idx++, _SelectionOrder++)
                    ImGuiSelectionBasicStorage_BatchSetItemSelected(
                        this, GetStorageIdFromIndex(idx), req.Selected,
                        size_before_amends, _SelectionOrder);
                ImGuiSelectionBasicStorage_BatchFinish(this, req.Selected,
                                                       size_before_amends);
            }
        } else if (req.Type == ImGuiSelectionRequestType_SetRange) {
            const int selection_changes =
                (int)req.RangeLastItem - (int)req.RangeFirstItem + 1;
            // ImGuiContext& g = *GImGui; IMGUI_DEBUG_LOG_SELECTION("Req %d/%d:
            // set %d to %d\n", ms_io->Requests.index_from_ptr(&req),
            // ms_io->Requests.Size, selection_changes, req.Selected);
            if (selection_changes == 1 || (selection_changes < Size / 100)) {
                ...
                // (sort sequential SetRange requests)
                for (int idx = (int)req.RangeFirstItem;
                     idx <= (int)req.RangeLastItem; idx++)
                    SetItemSelected(GetStorageIdFromIndex(idx), req.Selected);
            } else {
                // Append insertion + single sort likely be faster.
                // Use req.RangeDirection to set order field so that
                // shift+clicking from 1 to 5 is different than shift+clicking
                // from 5 to 1
                const int size_before_amends = _Storage.Data.Size;
                int selection_order =
                    _SelectionOrder +
                    ((req.RangeDirection < 0) ? selection_changes - 1 : 0);
                for (int idx = (int)req.RangeFirstItem;
...
// Apply requests coming from BeginMultiSelect() and EndMultiSelect().
// We also pull 'ms_io->ItemsCount' as passed for BeginMultiSelect() for
// consistency with ImGuiSelectionBasicStorage This makes no assumption about
// underlying storage.
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg) {
    ...
    // Fractional number of items helps seeing that we can scroll down/up
    // without looking at scrollbar.
    ImVec2 size = ImTrunc(CalcItemSize(size_arg, CalcItemWidth(),
    ...
    // FIXME-OPT: We could omit the BeginGroup() if label_size.x == 0.0f but
    // would need to omit the EndGroup() as well.
    BeginGroup();
    if (label_size.x > 0.0f) {
        ...
        RenderText(label_pos, label);
        window->DC.CursorMaxPos =
            ImMax(window->DC.CursorMaxPos, label_pos + label_size);
        AlignTextToFramePadding();
    }
...
void ImGui::EndListBox() {
    ...
    IM_ASSERT((window->Flags & ImGuiWindowFlags_ChildWindow) &&
              "Mismatched BeginListBox/EndListBox calls. Did you test the "
              "return value of BeginListBox?");
    IM_UNUSED(window);
...
bool ImGui::ListBox(const char* label, int* current_item,
    ...
    ImVec2 size(0.0f,
                ImTrunc(GetTextLineHeightWithSpacing() * height_in_items_f +
                        g.Style.FramePadding.y * 2.0f));

    if (!BeginListBox(label, size))
        return false;

    // Assume all items have even height (= 1 line of text). If you need items
    // of different height, you can create a custom version of ListBox() in your
    // code without using the clipper.
    bool value_changed = false;
    ImGuiListClipper clipper;
    clipper.Begin(
        items_count,
        GetTextLineHeightWithSpacing()); // We know exactly our line height here
                                         // so we pass it as a minor
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label,
    ...
    if (values_count >= values_count_min) {
        ...
        for (int n = 0; n < res_w; n++) {
            ...
            const float v1 = values_getter(data, (v1_idx + values_offset + 1) %
                                                     values_count);
            const ImVec2 tp1 =
...
float ImGuiMenuColumns::DeclColumns(float w_icon, float w_label,
                                    float w_shortcut, float w_mark) {
...
bool ImGui::BeginMenuBar() {
    ...
    // to the area below. However we clip with window full rect. We remove 1
    // worth of rounding to Max.x to that text in long menus and small windows
    // don't tend to display over the lower-right rounded area, which looks
...
void ImGui::EndMenuBar() {
    ...
    if (NavMoveRequestButNoResultYet() &&
        ...
        if (nav_earliest_child->ParentWindow == window &&
            ...
            // movement request for yet another frame. This involve a one-frame
            // delay which isn't very problematic in this situation. We could
            // remove it by scoring in advance for multiple window (probably not
            // worth bothering)
            ...
            g.NavDisableHighlight =
                true; // Hide highlight for the current frame so we don't see
                      // the intermediary selection.
            g.NavDisableMouseHover = g.NavMousePosDirty = true;
            NavMoveRequestForward(g.NavMoveDir, g.NavMoveClipDir,
                                  g.NavMoveFlags,
    ...
    EndGroup(); // Restore position on layer 0 // FIXME: Misleading to use a
                // group for that backup/restore
    window->DC.LayoutType = ImGuiLayoutType_Vertical;
    window->DC.IsSameLine = false;
...
bool ImGui::BeginViewportSideBar(const char* name, ImGuiViewport* viewport_p,
                                 ImGuiDir dir, float axis_size,
                                 ImGuiWindowFlags window_flags) {
    ...
    if (bar_window == NULL || bar_window->BeginCount == 0) {
        ...
        ImGuiViewportP* viewport =
            (ImGuiViewportP*)(void*)(viewport_p ? viewport_p
                                                : GetMainViewport());
        ImRect avail_rect = viewport->GetBuildWorkRect();
        ImGuiAxis axis = (dir == ImGuiDir_Up || dir == ImGuiDir_Down)
                             ? ImGuiAxis_Y
                             : ImGuiAxis_X;
        ...
        // Report our size into work area (for next frame) using actual window
        // size
    ...
    PushStyleVar(ImGuiStyleVar_WindowMinSize,
                 ImVec2(0, 0)); // Lift normal size constraint
...
bool ImGui::BeginMainMenuBar() {
    ...
    // FIXME: Consider removing support for safe area down the line... it's
    // messy. Nowadays consoles have support for TV calibration in OS settings.
    ...
    bool is_open = BeginViewportSideBar("##MainMenuBar", viewport, ImGuiDir_Up,
                                        height, window_flags);
...
void ImGui::EndMainMenuBar() {
    ...
    ImGuiContext& g = *GImGui;
    if (g.CurrentWindow == g.NavWindow && g.NavLayer == ImGuiNavLayer_Main &&
        !g.NavAnyRequest)
...
static bool IsRootOfOpenMenuSet() {
    ...
    // Initially we used 'upper_popup->OpenParentId == window->IDStack.back()'
    // to differentiate multiple menu sets from each others (e.g. inside menu
    // bar vs loose menu items) based on parent ID. This would however prevent
    // the use of e.g. PushID() user code submitting menus. Previously this
    // worked between popup and a first child menu because the first child menu
    // always had the _ChildWindow flag, making hovering on parent popup
    // possible while first child menu was focused - but this was generally a
    // bug with other side effects. Instead we don't treat Popup specifically
    // (in order to consistently support menu features in them), maybe the first
    // child menu of a Popup doesn't have the _ChildWindow flag, and we rely on
    // this IsRootOfOpenMenuSet() check to allow hovering between root
    // window/popup and first child menu. In the end, lack of ID check made it
    // so we could no longer differentiate between separate menu sets. To
    // compensate for that, we at least check parent window nav layer. This
    // fixes the most common case of menu opening on hover when moving between
    // window content and menu bar. Multiple different menu sets in same nav
    // layer would still open on hover, but that should be a lesser problem,
    // because if such menus are close in proximity in window content then it
    // won't feel weird and if they are far apart it likely won't be a problem
    // anyone runs into.
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled) {
    ...
    // on parent menu) The first menu in a hierarchy isn't so hovering doesn't
    // get across (otherwise e.g. resizing borders with
    // ImGuiButtonFlags_FlattenChildren would react), but top-most BeginMenu()
    ...
    // matching the behavior of Begin(). We are relying on a O(N) search - so
    // O(N log N) over the frame - which seems like the most efficient for the
    // expected small amount of BeginMenu() calls per frame. If somehow this is
    // ever becoming a problem we can switch to use e.g. ImGuiStorage mapping
    // key to last frame used.
    ...
    // wouldn't be able to hover parent without always being a Child window)
    // This is only done for items for the menu set and not the full parent
    // window.
    ...
    // find a suitable position for the child menu, However the final position
    // is going to be different! It is chosen by FindBestWindowPosForPopup().
    // e.g. Menus tend to overlap each other horizontally to amplify relative
    // Z-ordering.
    ...
    // We use ImGuiSelectableFlags_NoSetKeyOwner to allow down on one menu item,
    // move, up on another.
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        // direction. For ChildMenu, the popup position will be overwritten by
        // the call to FindBestWindowPosForPopup() in Begin()
        popup_pos =
            ImVec2(pos.x - 1.0f - IM_TRUNC(style.ItemSpacing.x * 0.5f),
        ...
        ImVec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel,
                        window->DC.CursorPos.y +
                            window->DC.CurrLineTextBaseOffset);
        pressed = Selectable("", menu_is_open, selectable_flags,
                             ImVec2(w, label_size.y));
        ...
        //  Only when they are other items sticking out we're going to add
        //  spacing, yet only register minimum width into the layout system.
    ...
    {
        // Close menu when not hovering it anymore unless we are moving roughly
        // in the direction of the menu Implement
        ...
        ImGuiWindow* child_menu_window =
            (child_popup && child_popup->Window &&
        ...
        if (g.HoveredWindow == window && child_menu_window != NULL) {
            ...
            ImVec2 tb = (child_dir > 0.0f) ? next_window_rect.GetTL()
                                           : next_window_rect.GetTR();
            ImVec2 tc = (child_dir > 0.0f) ? next_window_rect.GetBL()
                                           : next_window_rect.GetBR();
            const float pad_farmost_h =
                ImClamp(ImFabs(ta.x - tb.x) * 0.30f, ref_unit * 0.5f,
            ...
            tc.y =
                ta.y + ImMin((tc.y + pad_farmost_h) - ta.y, +ref_unit * 8.0f);
            moving_toward_child_menu =
                ImTriangleContainsPoint(ta, tb, tc, g.IO.MousePos);
            // GetForegroundDrawList()->AddTriangleFilled(ta, tb, tc,
            // moving_toward_child_menu ? IM_COL32(0,128,0,128) :
            // IM_COL32(128,0,0,128)); // [DEBUG]
        }

        // The 'HovereWindow == window' check creates an inconsistency (e.g.
        // moving away from menu slowly tends to hit same window, whereas moving
        // away fast does not) But we also need to not close the top-menu menu
        // when moving over void. Perhaps we should extend the triangle check to
        // a larger polygon. (Remember to test this on
        // BeginPopup("A")->BeginMenu("B") sequence which behaves slightly
        // differently as B isn't a Child of A and hovering isn't shared.)
        if (menu_is_open && !hovered && g.HoveredWindow == window &&
            !moving_toward_child_menu && !g.NavDisableMouseHover &&
            g.ActiveId == 0)
            want_close = true;

        // Open
        // (note: at this point 'hovered' actually includes the
        // NavDisableMouseHover == false test)
        if (!menu_is_open && pressed) // Click/activate to open
            ...
            want_open = true;
                 ...
                 g.MouseStationaryTimer >=
                     0.30f) // Hover to open (timer fallback)
            want_open = true;
        if (g.NavId == id &&
            ...
            menuset_is_open) // Click an open menu again to close it
        {
            ...
            want_open = menu_is_open = false;
                               ...
                               !menu_is_open)) // First click to open, then
    ...
    if (want_open && !menu_is_open &&
        ...
        // Don't reopen/recycle same menu level in the same frame if it is a
        // different menu ID, first close the other menu and yield for a frame.
        OpenPopup(label);
    } else if (want_open) {
        menu_is_open = true;
        OpenPopup(
    ...
    if (menu_is_open) {
        ...
        if (menu_is_open) {
            ...
            // Perform an init request in the case the popup was already open
            // (via a previous mouse hover)
            ...
            // IsItemHovered(), but IsItemHovered() also relies on its support
...
void ImGui::EndMenu() {
    ...
    IM_ASSERT(window->Flags &
              ImGuiWindowFlags_Popup); // Mismatched BeginMenu()/EndMenu() calls
...
bool ImGui::MenuItemEx(const char* label, const char* icon,
    ...
    // We've been using the equivalent of ImGuiSelectableFlags_SetNavIdOnHover
    // on all Selectable() since early Nav system days (commit 43ee5d73), but I
    // am unsure whether this should be kept at all. For now moved it to be an
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        // Note that in this situation: we don't render the shortcut, we render
        // a highlight instead of the selected tick mark.
        float w = label_size.x;
        window->DC.CursorPos.x += IM_TRUNC(style.ItemSpacing.x * 0.5f);
        ImVec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel,
                        window->DC.CursorPos.y +
        ...
        if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible) {
            ...
            if (selected)
                RenderCheckMark(window->DrawList,
                                pos + ImVec2(offsets->OffsetMark + stretch_w +
                                                 g.FontSize * 0.40f,
...
static void TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiID tab_id,
...
bool ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags) {
    ...
    tab_bar->SeparatorMinX =
        tab_bar->BarRect.Min.x - IM_TRUNC(window->WindowPadding.x * 0.5f);
    tab_bar->SeparatorMaxX =
        tab_bar->BarRect.Max.x + IM_TRUNC(window->WindowPadding.x * 0.5f);
...
bool ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb,
    ...
    // Ensure correct ordering when toggling ImGuiTabBarFlags_Reorderable flag,
    // or when a new tab was added while being not reorderable
    ...
    window->DC.CursorPos = ImVec2(
        tab_bar->BarRect.Min.x, tab_bar->BarRect.Max.y + tab_bar->ItemSpacingY);

    // Draw separator
    // (it would be misleading to draw this in EndTabBar() suggesting that it
    // may be drawn over tabs, as tab bar are appendable)
    ...
    if (g.Style.TabBarBorderSize > 0.0f) {
        ...
        window->DrawList->AddRectFilled(
            ImVec2(tab_bar->SeparatorMinX, y - g.Style.TabBarBorderSize),
...
// not scrolling)
static float TabBarCalcScrollableWidth(ImGuiTabBar* tab_bar,
                                       ImGuiTabBarSection* sections) {
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar) {
    ...
    for (int tab_src_n = 0; tab_src_n < tab_bar->Tabs.Size; tab_src_n++) {
        ...
        // We will need sorting if tabs have changed section (e.g. moved from
        // one of Leading/Central/Trailing to another)
    ...
    // anymore, so layout the shrink data as: leading, trailing, central
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
        ...
        // Additionally, when using TabBarAddTab() to manipulate tab bar order
        // we occasionally insert new tabs that don't have a width yet, and we
        // cannot wait for the next BeginTabItem() call. We cannot compute this
        // width within TabBarAddTab() because font size depends on the active
        // window.
        ...
        // Store data so we can build an array sorted by width if we need to
        // shrink tabs down
    ...
    if ((tab_bar->WidthAllTabsIdeal > tab_bar->BarRect.GetWidth() &&
        ...
        if (ImGuiTabItem* scroll_and_select_tab =
    ...
    if (central_section_is_visible)
        width_excess = ImMax(section_1_w - (tab_bar->BarRect.GetWidth() -
    ...
    for (int section_n = 0; section_n < 3; section_n++) {
        ...
        if (section_n == 2)
            tab_offset =
                ImMin(ImMax(0.0f, tab_bar->BarRect.GetWidth() - section->Width),
                      tab_offset);
    ...
    // If we have lost the selected tab, select the next most recently active
    // one
    ...
    if (scroll_to_tab_id != 0)
        ...
        const ImGuiKey wheel_key = g.IO.MouseWheelRequestAxisSwap
        ...
        if (TestKeyOwner(wheel_key, tab_bar->ID) && wheel != 0.0f) {
            ...
            tab_bar->ScrollingTarget = TabBarScrollClamp(
                tab_bar, tab_bar->ScrollingTarget - scroll_step);
        }
    ...
    if (tab_bar->ScrollingAnim != tab_bar->ScrollingTarget) {
        // Scrolling speed adjust itself so we can always reach our target in
        // 1/3 seconds. Teleport if we are aiming far off the visible line
    ...
    // Actual layout in host window (we don't do it in BeginTabBar() so as not
    // to waste an extra frame)
    ImGuiWindow* window = g.CurrentWindow;
    window->DC.CursorPos = tab_bar->BarRect.Min;
    ItemSize(ImVec2(tab_bar->WidthAllTabs, tab_bar->BarRect.GetHeight()),
...
void ImGui::TabBarCloseTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab) {
    ...
    if ((tab->Flags & (ImGuiTabItemFlags_UnsavedDocument |
        ...
        // However we don't run it in the case where the 'Unsaved' flag is set,
        // so user gets a chance to fully undo the closure
        tab->WantClose = true;
        if (tab_bar->VisibleTabId == tab->ID) {
...
static float ImGui::TabBarScrollClamp(ImGuiTabBar* tab_bar, float scrolling) {
    scrolling =
        ImMin(scrolling, tab_bar->WidthAllTabs - tab_bar->BarRect.GetWidth());
...
static void ImGui::TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiID tab_id,
    ...
    ImGuiContext& g = *GImGui;
    float margin =
        g.FontSize * 1.0f; // When to scroll to make Tab N+1 visible always make
                           // a bit of N visible to suggest more scrolling area
...
void ImGui::TabBarQueueReorder(ImGuiTabBar* tab_bar, ImGuiTabItem* tab,
                               int offset) {
    IM_ASSERT(offset != 0);
...
void ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar,
    ...
    const bool is_central_section =
        (src_tab->Flags & ImGuiTabItemFlags_SectionMask_) == 0;
    ...
    for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir) {
        ...
        // Include spacing after tab, so when mouse cursor is between tabs we
        // would not continue checking further tabs that are not hovered.
        const float x1 =
            bar_offset + dst_tab->Offset - g.Style.ItemInnerSpacing.x;
        const float x2 = bar_offset + dst_tab->Offset + dst_tab->Width +
                         g.Style.ItemInnerSpacing.x;
        // GetForegroundDrawList()->AddRect(ImVec2(x1, tab_bar->BarRect.Min.y),
        // ImVec2(x2, tab_bar->BarRect.Max.y), IM_COL32(255, 0, 0, 255));
...
bool ImGui::TabBarProcessReorder(ImGuiTabBar* tab_bar) {
    ...
    // IM_ASSERT(tab_bar->Flags & ImGuiTabBarFlags_Reorderable); // <- this may
    ...
    ImGuiTabItem* src_tab =
        (tab_bar->ReorderRequestOffset > 0) ? tab1 + 1 : tab2;
    ImGuiTabItem* dst_tab =
        (tab_bar->ReorderRequestOffset > 0) ? tab1 : tab2 + 1;
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar) {
    ...
    const ImVec2 arrow_button_size(g.FontSize - 2.0f,
    ...
    if (select_dir != 0)
        if (ImGuiTabItem* tab_item =
            ...
            // Skip tab item buttons until another tab item is found or end is
            // reached
            while (tab_to_scroll_to == NULL) {
                ...
                // (even if first/last item is a button, return it so we can
                // update the scroll)
...
bool ImGui::BeginTabItem(const char* label, bool* p_open,
    ...
    IM_ASSERT(!(
        flags &
        ImGuiTabItemFlags_Button)); // BeginTabItem() Can't be used with button
    ...
    if (ret && !(flags & ImGuiTabItemFlags_NoPushId)) {
        ...
        PushOverrideID(tab->ID); // We already hashed 'label' so push into the
                                 // ID stack directly instead of doing another
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open,
    ...
    // render. We make a call to ItemAdd() so that attempts to use a contextual
    // popup menu with an implicit ID won't use an older ID.
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    if (p_open && !*p_open) {
    ...
    IM_ASSERT(
        (flags & (ImGuiTabItemFlags_Leading | ImGuiTabItemFlags_Trailing)) !=
    ...
    // ImGuiTabItemFlags_NoCloseButton passed by user (although not documented)
    ...
    ImVec2 size = TabItemCalcSize(
        label, (p_open != NULL) || (flags & ImGuiTabItemFlags_UnsavedDocument));
    ...
    // tab bar stops being submitted and then gets submitted again, the tabs
    // will have 'tab_appearing=true' but 'tab_is_new=false'.
    ...
    if (want_clip_rect)
        PushClipRect(
            ImVec2(ImMax(bb.Min.x, tab_bar->ScrollingRectMinX), bb.Min.y - 1),
    ...
    if (held && !tab_appearing && IsMouseDragging(0)) {
        if (!g.DragDropActive &&
            (tab_bar->Flags & ImGuiTabBarFlags_Reorderable)) {
            // While moving a tab it will jump on the other side of the mouse,
            // so we also test for MouseDelta.x
    ...
    const ImU32 tab_col = GetColorU32(
        ...
        : tab_contents_visible
            ? (tab_bar_focused ? ImGuiCol_TabSelected
                               : ImGuiCol_TabDimmedSelected)
            : (tab_bar_focused ? ImGuiCol_Tab : ImGuiCol_TabDimmed));
    TabItemBackground(display_draw_list, bb, flags, tab_col);
    if (tab_contents_visible &&
        (tab_bar->Flags & ImGuiTabBarFlags_DrawSelectedOverline) &&
        ...
        display_draw_list->AddLine(
            bb.GetTL() + ImVec2(x_offset, y_offset),
            bb.GetTR() + ImVec2(-x_offset, y_offset),
            GetColorU32(tab_bar_focused ? ImGuiCol_TabSelectedOverline
    ...
    if (text_clipped && g.HoveredId == id && !held)
        if (!(tab_bar->Flags & ImGuiTabBarFlags_NoTooltip) &&
            ...
            SetItemTooltip("%.*s", (int)(FindRenderedTextEnd(label) - label),
                           label);
...
void ImGui::SetTabItemClosed(const char* label) {
    ...
    if (is_within_manual_tab_bar) {
        ...
        if (ImGuiTabItem* tab = TabBarFindTabByID(tab_bar, tab_id))
            tab->WantClose =
                true; // Will be processed by next call to TabBarLayout()
    }
}
...
ImVec2 ImGui::TabItemCalcSize(const char* label,
    ...
    if (has_close_button_or_unsaved_marker)
        size.x += g.Style.FramePadding.x +
                  (g.Style.ItemInnerSpacing.x +
                   g.FontSize); // We use Y intentionally to fit the close
                                // button circle.
    else
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb,
    ...
    const float rounding = ImMax(0.0f, ImMin((flags & ImGuiTabItemFlags_Button)
                                                 ? g.Style.FrameRounding
                                                 : g.Style.TabRounding,
                                             width * 0.5f - 1.0f));
    ...
    if (g.Style.TabBorderSize > 0.0f) {
        ...
        draw_list->PathArcToFast(
            ImVec2(bb.Min.x + rounding + 0.5f, y1 + rounding + 0.5f), rounding,
            6, 9);
        draw_list->PathArcToFast(
            ImVec2(bb.Max.x - rounding - 0.5f, y1 + rounding + 0.5f), rounding,
            9, 12);
        draw_list->PathLineTo(ImVec2(bb.Max.x - 0.5f, y2));
        draw_list->PathStroke(GetColorU32(ImGuiCol_Border), 0,
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb,
    ...
    if (out_text_clipped) {
        *out_text_clipped = (text_ellipsis_clip_bb.Min.x + label_size.x) >
        ...
        // *out_text_clipped ? IM_COL32(255, 0, 0, 255) : IM_COL32(0, 255, 0,
        // 255));
    }

    const float button_sz = g.FontSize;
    const ImVec2 button_pos(
        ImMax(bb.Min.x, bb.Max.x - frame_padding.x - button_sz),
        bb.Min.y + frame_padding.y);

    // Close Button & Unsaved Marker
    // We are relying on a subtle and confusing distinction between 'hovered'
    // and 'g.HoveredId' which happens because we are using
    ...
    if (close_button_id != 0)
        if (is_contents_visible ||
            bb.GetWidth() >=
                ImMax(button_sz, g.Style.TabMinWidthForCloseButton))
    ...
    bool unsaved_marker_visible =
        (flags & ImGuiTabItemFlags_UnsavedDocument) != 0 &&
    ...
    if (close_button_visible) {
        ...
        const ImRect bullet_bb(button_pos,
                               button_pos + ImVec2(button_sz, button_sz));
        RenderBullet(draw_list, bullet_bb.GetCenter(),
                     GetColorU32(ImGuiCol_Text));
    }
