...
// ScrollX/ScrollY flags. Default value is ImVec2(0.0f, 0.0f).
//   X
...
//   - ImGuiTableFlags_NoHostExtendX -> Make outer width auto-fit to columns
//   (overriding outer_size.x value). Only available when ScrollX/ScrollY are
//   disabled and Stretch columns are not used.
//   - ImGuiTableFlags_NoHostExtendY -> Make outer height stop exactly at
...
// (Reference: ImGuiTableFlags_SizingXXX flags and
// ImGuiTableColumnFlags_WidthXXX flags)
//-----------------------------------------------------------------------------
...
//   - with ImGuiTableColumnFlags_WidthFixed,    init_width  <= 0 (default)  -->
//   width is automatic
//   - with ImGuiTableColumnFlags_WidthFixed,    init_width  >  0 (explicit) -->
//   width is custom
//   - with ImGuiTableColumnFlags_WidthStretch,  init_weight <= 0 (default)  -->
//   weight is 1.0f
//   - with ImGuiTableColumnFlags_WidthStretch,  init_weight >  0 (explicit) -->
//   weight is custom
...
// ImGuiTableColumnFlags_WidthXXXX flag)
//   - with Table policy ImGuiTableFlags_SizingFixedFit      --> default Column
//   policy is ImGuiTableColumnFlags_WidthFixed, default Width is equal to
//   contents width
//   - with Table policy ImGuiTableFlags_SizingFixedSame     --> default Column
//   policy is ImGuiTableColumnFlags_WidthFixed, default Width is max of all
//   contents width
//   - with Table policy ImGuiTableFlags_SizingStretchSame   --> default Column
//   policy is ImGuiTableColumnFlags_WidthStretch, default Weight is 1.0f
//   - with Table policy ImGuiTableFlags_SizingStretchWeight --> default Column
//   policy is ImGuiTableColumnFlags_WidthStretch, default Weight is
...
//   - when using ImGuiTableFlags_SizingFixedSame with mixed columns, only the
//   Fixed/Auto columns will match their widths to the width of the maximum
//   contents.
//   - when using ImGuiTableFlags_SizingStretchSame with mixed columns, only the
...
static const int TABLE_DRAW_CHANNEL_NOCLIP =
    2; // When using ImGuiTableFlags_NoClip (this becomes the last visible
       // channel)
...
inline ImGuiTableFlags TableFixFlags(ImGuiTableFlags flags,
    ...
    // Adjust flags: disable NoHostExtendX/NoHostExtendY if we have any
    // scrolling going on
...
bool ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count,
    ...
    if (outer_window->SkipItems) // Consistent with other tables + beneficial
                                 // side effect that assert on miscalling
    ...
    // - instance numbers varying/unstable. may not be a direct problem for
    // users, but could make outside access broken or confusing, e.g.
    // TestEngine.
    // - can't implement support for ImGuiChildFlags_ResizeY as we need to
    // somehow pull the height data from somewhere. this also needs stable
    // instance numbers. The side-effects of accessing table data on coarse clip
    // would be:
    // - always reserving the pooled ImGuiTable data ahead for a fully clipped
    // table (minor IMHO). Also the 'outer_window_is_measuring_size' criteria
    // may already be defeating this in some situations.
    ...
    // contents.
    if (use_child_window) {
        // Ensure no vertical scrollbar appears if we only want horizontal one,
        // to make flag consistent (we have no other way to disable vertical
        // scrollbar of a window while keeping the horizontal one showing)
        ImVec2 override_content_size(FLT_MAX, FLT_MAX);
        if ((flags & ImGuiTableFlags_ScrollX) &&
            !(flags & ImGuiTableFlags_ScrollY))
            override_content_size.y = FLT_MIN;

        // Ensure specified width (when not specified, Stretched columns will
        // act as if the width == OuterWidth and never lead to any scrolling).
        // We don't handle inner_width < 0.0f, we could potentially use it to
        // right-align based on the right side of the child window work rect,
        // which would require knowing ahead if we are going to have decoration
        // taking horizontal spaces (typically a vertical scrollbar).
        ...
        if (override_content_size.x != FLT_MAX ||
            ...
            SetNextWindowContentSize(ImVec2(
                override_content_size.x != FLT_MAX ? override_content_size.x
                                                   : 0.0f,
                override_content_size.y != FLT_MAX ? override_content_size.y
                                                   : 0.0f));

        // Reset scroll if we are reactivating it
        if ((previous_flags &
             (ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY)) == 0)
        ...
        BeginChildEx(name, instance_id, outer_rect.GetSize(),
                     ImGuiChildFlags_None, child_window_flags);
        ...
        // horizontal decorations (aka vertical scrollbar) so stretched columns
        // can be aligned)
        if (instance_no == 0) {
    ...
    // on TableMergeDrawChannels() expanding non-clipped column toward the
    // limits of that rectangle, in order for ImDrawListSplitter::Merge() to
    // merge the draw commands. However since the overlap problem only affect
    // scrolling tables in this case we can get away with doing it without extra
    // cost).
    if (inner_window != outer_window) {
        if (flags & ImGuiTableFlags_BordersOuterV)
            table->HostClipRect.Min.x =
                ImMin(table->HostClipRect.Min.x + TABLE_BORDER_SIZE,
                      table->HostClipRect.Max.x);
        if (flags & ImGuiTableFlags_BordersOuterH)
            table->HostClipRect.Min.y =
                ImMin(table->HostClipRect.Min.y + TABLE_BORDER_SIZE,
    ...
    if (inner_window != outer_window) // So EndChild() within the inner window
                                      // can restore the table properly.
    ...
    if (table->IsInitializing) {
        ...
        for (int n = 0; n < columns_count; n++) {
            ...
            if (old_columns_to_preserve && n < old_columns_count) {
                // FIXME: We don't attempt to preserve column order in this
                // path.
    ...
    // fonts with mixed results. It doesn't need to be perfect but merely
    // provide a decent transition.
    ...
    if (table->RefScale != 0.0f && table->RefScale != new_ref_scale_unit) {
        ...
        // IMGUI_DEBUG_PRINT("[table] %08X RefScaleUnit %.3f -> %.3f, scaling
    ...
    // leading to the TableUpdateLayout() call.. This is not strictly necessary
    // but will reduce cases were "out of table" output will be misleading to
    // the user. Because we cannot safely assert in EndTable() when no rows have
    // been created, this seems like our best option.
...
void ImGui::TableBeginInitMemory(ImGuiTable* table, int columns_count) {
    ...
    const int columns_bit_array_size =
        (int)ImBitArrayGetStorageSizeInBytes(columns_count);
    ...
    table->EnabledMaskByDisplayOrder =
        (ImU32*)span_allocator.GetSpanPtrBegin(3);
...
void ImGui::TableBeginApplyRequests(ImGuiTable* table) {
    ...
    if (table->InstanceCurrent == 0) {
        ...
        // Process auto-fit for single column, which is a special case for
        // stretch columns and fixed columns with FixedSame policy.
    ...
    if (table->InstanceCurrent == 0) {
        ...
        if (table->ReorderColumn != -1 && table->ReorderColumnDir != 0) {
            ...
            // In the configuration below, moving C to the right of E will lead
            // to:
            ...
            for (int order_n = src_order + reorder_dir;
                ...
                table->Columns[table->DisplayOrderToIndex[order_n]]
                    .DisplayOrder -= (ImGuiTableColumnIdx)reorder_dir;
...
static void TableSetupColumnFlags(ImGuiTable* table, ImGuiTableColumn* column,
    ...
    if ((flags & ImGuiTableColumnFlags_WidthMask_) == 0) {
        const ImGuiTableFlags table_sizing_policy =
...
void ImGui::TableUpdateLayout(ImGuiTable* table) {
    ...
    ImBitArrayClearAllBits(table->EnabledMaskByDisplayOrder,
                           table->ColumnsCount);
    table->LeftMostEnabledColumn = -1;
    table->MinColumnWidth =
        ImMax(1.0f, g.Style.FramePadding.x *
                        1.0f); // g.Style.ColumnsMinSpacing; // FIXME-TABLE

    // [Part 1] Apply/lock Enabled and Order states. Calculate auto/ideal width
    // for columns. Count fixed/stretch columns. Process columns in their
    // visible orders as we are building the Prev/Next indices.
    int count_fixed = 0; // Number of columns that have fixed sizing policies
    int count_stretch =
        0; // Number of columns that have stretch sizing policies
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
        ...
        // mandatory to call TableSetupColumn() every frame. It would easily
        // work without but we're not ready to guarantee it since e.g. names
        // need resubmission anyway. We take a slight shortcut but in theory we
        // could be calling TableSetupColumn() here with dummy values, it should
        ...
        column->IsEnabled =
            column->IsUserEnabled &&
            (column->Flags & ImGuiTableColumnFlags_Disabled) == 0;

        if (column->SortOrder != -1 && !column->IsEnabled)
            table->IsSortSpecsDirty = true;
        if (column->SortOrder > 0 &&
            !(table->Flags & ImGuiTableFlags_SortMulti))
            table->IsSortSpecsDirty = true;
        ...
        ImBitArraySetBit(table->EnabledMaskByDisplayOrder,
                         column->DisplayOrder);
        prev_visible_column_idx = column_n;
        IM_ASSERT(column->IndexWithinEnabledSet <= column->DisplayOrder);

        // Calculate ideal/auto column width (that's the width required for all
        // contents to be visible without clipping) Combine width from regular
        // rows + width from headers unless requested not to.
        ...
        if (column->Flags & ImGuiTableColumnFlags_WidthStretch) {
            ...
            fixed_max_width_auto =
                ImMax(fixed_max_width_auto, column->WidthAuto);
            count_fixed++;
        }
    }
    if ((table->Flags & ImGuiTableFlags_Sortable) &&
    ...
    table->RightMostEnabledColumn =
        (ImGuiTableColumnIdx)prev_visible_column_idx;
    IM_ASSERT(table->LeftMostEnabledColumn >= 0 &&
              table->RightMostEnabledColumn >= 0);

    // [Part 2] Disable child window clipping while fitting columns. This is not
    // strictly necessary but makes it possible to avoid the column fitting
    // having to wait until the first visible frame of the child container (may
    // or not be a good thing). Also see #6510.
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
        ...
        if (column->Flags & ImGuiTableColumnFlags_WidthFixed) {
            ...
            else if ((column->Flags & ImGuiTableColumnFlags_WidthFixed) &&
                     !column_is_resizable && column->IsRequestOutput)
                column->WidthRequest = width_auto;

            // FIXME-TABLE: Increase minimum size during init frame to avoid
            // biasing auto-fitting widgets (e.g. TextWrapped) too much.
            // Otherwise what tends to happen is that TextWrapped would output a
            // very large height (= first frame scrollbar display very off +
            // clipper would skip lots of items). This is merely making the
            // side-effect less extreme, but doesn't properly fixes it.
            // FIXME: Move this to ->WidthGiven to avoid temporary lossyless?
            // FIXME: This break IsPreserveWidthAuto from not flickering if the
            // stored WidthAuto was smaller.
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
        ...
        if (column->Flags & ImGuiTableColumnFlags_WidthStretch) {
            ...
            column->WidthRequest =
                IM_TRUNC(ImMax(width_avail_for_stretched_columns * weight_ratio,
                               table->MinColumnWidth) +
                         0.01f);
            width_remaining_for_stretched_columns -= column->WidthRequest;
        }

        // [Resize Rule 1] The right-most Visible column is not resizable if
        // there is at least one Stretch column See additional comments in
        // TableSetColumnWidth().
        ...
        column->WidthGiven =
            ImTrunc(ImMax(column->WidthRequest, table->MinColumnWidth));
    ...
    const bool is_hovering_table =
        ItemHoverable(mouse_hit_rect, 0, ImGuiItemFlags_None);
    ...
    if (table->AngledHeadersHeight > 0.0f)
        if (g.IO.MousePos.y >= table->OuterRect.Min.y &&
            ...
            mouse_skewed_x +=
                ImTrunc((table->OuterRect.Min.y + table->AngledHeadersHeight -
                         g.IO.MousePos.y) *
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
        ...
        column->NavLayerCurrent =
            (ImS8)(table->FreezeRowsCount > 0
                       ? ImGuiNavLayer_Menu
                       : ImGuiNavLayer_Main); // Use Count NOT request so Header
        ...
        // - ClipRect.Max.x: using WorkMaxX instead of MaxX (aka including
        // padding) makes things more consistent when resizing down, tho
        // slightly detrimental to visibility in very-small column.
        // - ClipRect.Max.x: using MaxX makes it easier for header to receive
        // hover highlight with no discontinuity and display sorting arrow.
        // - FIXME-TABLE: We want equal width columns to have equal
        // (ClipRect.Max.x - WorkMinX) width, which means ClipRect.max.x cannot
        // stray off host_clip_rect.Max.x else right-most column may appear
        // shorter.
        ...
        // Note that scrolling tables (where inner_window != outer_window)
        // handle Y clipped earlier in BeginTable() so IsVisibleY really only
        // applies to non-scrolling tables.
        ...
        // scroll line as not IsVisibleY. Taking advantage of LastOuterHeight
        // would yield good results there...
        // FIXME-TABLE: Y clipping is disabled because it effectively means not
        // submitting will reduce contents width which is fed to
        // outer_window->DC.CursorMaxPos.x, and this may be used (e.g. typically
        // by outer_window using AlwaysAutoResize or outer_window's horizontal
        // scrollbar, but could be something else). Possible solution to
        // preserve last known content width for clipped column. Test
        // 'table_reported_size' fails when enabling Y clipping and window is
        // resized small.
        ...
        // per-cell, and therefore isn't useful in many cases (to be able to
        // honor this we might be able to store a log of cells width, per row,
        // for visible rows, but nav/programmatic scroll would have visible
        // artifacts.)
        // if (column->Flags & ImGuiTableColumnFlags_AlignRight)
        //    column->WorkMinX = ImMax(column->WorkMinX, column->MaxX -
        //    column->ContentWidthRowsUnfrozen);
        // else if (column->Flags & ImGuiTableColumnFlags_AlignCenter)
        //    column->WorkMinX = ImLerp(column->WorkMinX, ImMax(column->StartX,
        ...
        if (table->InstanceCurrent == 0) {
            ...
            // As we store an absolute value to make per-cell updates faster, we
            // need to offset values used for width computation.
        ...
        // Don't decrement auto-fit counters until container window got a chance
        // to submit its items
        ...
        if (visible_n < table->FreezeColumnsCount)
            host_clip_rect.Min.x =
                ImClamp(column->MaxX + TABLE_BORDER_SIZE, host_clip_rect.Min.x,
    ...
    // In case the table is visible (e.g. decorations) but all columns clipped,
    // we keep a column visible. Else if give no chance to a clipper-savy user
    // to submit rows and therefore total contents height used by scrollbar.
    ...
    if (table->Flags & ImGuiTableFlags_NoHostExtendX) {
        ...
        table->InnerClipRect.Max.x =
            ImMin(table->InnerClipRect.Max.x, unused_x1);
    }
    ...
    if (table->Flags & ImGuiTableFlags_NoHostExtendY)
        ...
        window_content_max_y = ImMax(
            table->InnerWindow->ContentRegionRect.Max.y,
            (table->Flags & ImGuiTableFlags_ScrollY) ? 0.0f
                                                     : table->OuterRect.Max.y);
    table->InnerWindow->WorkRect.Max.y = ImClamp(
    ...
    // - You may call TableDrawDefaultContextMenu() with selected flags to
    // display specific sections of the default menu,
    //   e.g. TableDrawDefaultContextMenu(table, table->Flags &
    //   ~ImGuiTableFlags_Hideable) will display everything EXCEPT columns
    //   visibility options.
    ...
    // [Part 12] Sanitize and build sort specs before we have a chance to use
    // them for display. This path will only be exercised when sort specs are
    // modified before header rows (e.g. init or visibility change)
    if (table->IsSortSpecsDirty && (table->Flags & ImGuiTableFlags_Sortable))
        TableSortSpecsBuild(table);

    // [Part 13] Setup inner window decoration size (for scrolling / nav
    // tracking to properly take account of frozen rows/columns)
    if (table->FreezeColumnsRequest > 0)
        table->InnerWindow->DecoInnerSizeX1 =
            table
                ->Columns[table->DisplayOrderToIndex
...
void ImGui::TableUpdateBorders(ImGuiTable* table) {
    ...
    // At this point OuterRect height may be zero or under actual final height,
    // so we rely on temporal coherency and use the final height from last
    // frame. Because this is only affecting _interaction_ with columns, it is
    // not really problematic (whereas the actual visual will be displayed in
    // EndTable() and using the current frame height). Actual columns
    // highlight/render will be performed in EndTable() and not be affected.
    ImGuiTableInstanceData* table_instance =
        TableGetInstanceData(table, table->InstanceCurrent);
    const float hit_half_width =
    ...
    const float hit_y2_body =
        ImMax(table->OuterRect.Max.y, hit_y1 + table_instance->LastOuterHeight -
                                          table->AngledHeadersHeight);
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
        ...
        const float border_y2_hit =
            (table->Flags & ImGuiTableFlags_NoBordersInBody) ? hit_y2_head
                                                             : hit_y2_body;
        ...
        if ((hovered &&
             g.HoveredIdTimer > TABLE_RESIZE_SEPARATOR_FEEDBACK_TIMER) ||
            held) {
...
void ImGui::EndTable() {
    ...
    // (instead of just hoping that TableBegin/TableEnd will work), get borders
    // drawn, etc.
    ...
    if (table->Flags & ImGuiTableFlags_ScrollX) {
        const float outer_padding_for_border =
            (table->Flags & ImGuiTableFlags_BordersOuterV) ? TABLE_BORDER_SIZE
                                                           : 0.0f;
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        if (IM_BITARRAY_TESTBIT(table->EnabledMaskByIndex, column_n)) {
            ...
            if ((column->Flags & ImGuiTableColumnFlags_WidthStretch) &&
                (column->Flags & ImGuiTableColumnFlags_NoResize) != 0)
                auto_fit_width_for_stretched_min = ImMax(
    ...
    if (table->ResizedColumn != -1 &&
        ...
        const float new_x2 = (g.IO.MousePos.x - g.ActiveIdClickOffset.x +
                              ImTrunc(TABLE_RESIZE_SEPARATOR_HALF_THICKNESS *
                                      g.CurrentDpiScale));
        const float new_width =
            ImTrunc(new_x2 - column->MinX - table->CellSpacingX1 -
    ...
    if (inner_window != outer_window) {
        ...
        inner_window->DC.NavLayersActiveMask |=
            1 << ImGuiNavLayer_Main; // So empty table don't appear to navigate
                                     // differently.
        EndChild();
    ...
    if (table->Flags & ImGuiTableFlags_NoHostExtendX) {
        ...
        // ColumnsAutoFitWidth may be one frame ahead here since for
        // Fixed+NoResize is calculated from latest contents
        ...
        // Some references for this: #7651 + tests "table_reported_size",
        // "table_reported_size_outer" equivalent Y block
        // - Checking for ImGuiTableFlags_ScrollX/ScrollY flag makes us a frame
        // ahead when disabling those flags.
        // - FIXME-TABLE: Would make sense to pre-compute expected scrollbar
        // visibility/sizes to generally save a frame of feedback.
        const float inner_content_max_x =
            table->OuterRect.Min.x +
            table
                ->ColumnsAutoFitWidth; // Slightly misleading name but used for
                                       // code symmetry with inner_content_max_y
        const float decoration_size = table->TempData->AngledHeadersExtraWidth +
                                      ((table->Flags & ImGuiTableFlags_ScrollY)
                                           ...
                                           : 0.0f);
        outer_window->DC.IdealMaxPos.x = ImMax(
        ...
        outer_window->DC.CursorMaxPos.x =
            ImMax(backup_outer_max_pos.x, table->OuterRect.Max.x);
    }
    if (temp_data->UserOuterSize.y <= 0.0f) {
        const float decoration_size = (table->Flags & ImGuiTableFlags_ScrollX)
                                          ? inner_window->ScrollbarSizes.y
                                          : 0.0f;
        outer_window->DC.IdealMaxPos.y = ImMax(
        ...
        outer_window->DC.CursorMaxPos.y =
            ImMax(backup_outer_max_pos.y,
                  ImMin(table->OuterRect.Max.y,
                        inner_content_max_y + decoration_size));
    } else {
        // OuterRect.Max.y may already have been pushed downward from the
        // initial value (unless ImGuiTableFlags_NoHostExtendY is set)
        outer_window->DC.CursorMaxPos.y =
            ImMax(backup_outer_max_pos.y, table->OuterRect.Max.y);
    }
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags,
    ...
    // default, to avoid storing width into weight and vice-versa. Give a grace
    // to users of ImGuiTableFlags_ScrollX.
    if (table->IsDefaultSizingPolicy &&
        (flags & ImGuiTableColumnFlags_WidthMask_) == 0 &&
    ...
    if (table->IsInitializing) {
        ...
        if (flags & ImGuiTableColumnFlags_DefaultSort &&
            ...
            column->SortOrder =
                0; // Multiple columns using _DefaultSort will be reassigned
                   // unique SortOrder values when building the sort specs.
...
void ImGui::TableSetupScrollFreeze(int columns, int rows) {
    ...
    // Ensure frozen columns are ordered in their section. We still allow
    // multiple frozen columns to be reordered.
    ...
    for (int column_n = 0; column_n < table->FreezeColumnsRequest; column_n++) {
        ...
        if (order_n != column_n && order_n >= table->FreezeColumnsRequest) {
            ImSwap(table->Columns[table->DisplayOrderToIndex[order_n]]
                       .DisplayOrder,
                   table->Columns[table->DisplayOrderToIndex[column_n]]
                       .DisplayOrder);
            ImSwap(table->DisplayOrderToIndex[order_n],
...
// - Require table to have the ImGuiTableFlags_Hideable flag because we are
...
// - Alternative: the ImGuiTableColumnFlags_Disabled is an overriding/master
...
ImGuiID ImGui::TableGetColumnResizeID(ImGuiTable* table, int column_n,
                                      int instance_no) {
    IM_ASSERT(column_n >= 0 && column_n < table->ColumnsCount);
...
void ImGui::TableSetBgColor(ImGuiTableBgTarget target, ImU32 color,
                            int column_n) {
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
...
void ImGui::TableBeginRow(ImGuiTable* table) {
    ...
    window->DC.PrevLineSize = window->DC.CurrLineSize = ImVec2(
...
void ImGui::TableEndRow(ImGuiTable* table) {
    ...
    if ((table->RowFlags & ImGuiTableRowFlags_Headers) &&
        (table->CurrentRow == 0 ||
         (table->LastRowFlags & ImGuiTableRowFlags_Headers)))
    ...
    if (is_visible) {
        ...
        if (table->CurrentRow > 0 &&
            (table->Flags & ImGuiTableFlags_BordersInnerH))
        ...
        if ((bg_col0 | bg_col1 | top_border_col) != 0 ||
            ...
            // In theory we could call SetWindowClipRectBeforeSetChannel() but
            // since we know TableEndRow() is always followed by a change of
            // clipping rectangle we perform the smallest overwrite possible
            // here.
            if ((table->Flags & ImGuiTableFlags_NoClip) == 0)
        ...
        // Draw row background
        // We soft/cpu clip this so all backgrounds and borders can share the
        // same clipping rectangle
        if (bg_col0 || bg_col1) {
            ImRect row_rect(table->WorkRect.Min.x, bg_y1, table->WorkRect.Max.x,
                            bg_y2);
            row_rect.ClipWith(table->BgClipRect);
            if (bg_col0 != 0 && row_rect.Min.y < row_rect.Max.y)
                window->DrawList->AddRectFilled(row_rect.Min, row_rect.Max,
                                                bg_col0);
        ...
        if (draw_cell_bg_color) {
            ImGuiTableCellData* cell_data_end =
                &table->RowCellData[table->RowCellDataCurrent];
            for (ImGuiTableCellData* cell_data = &table->RowCellData[0];
                 cell_data <= cell_data_end; cell_data++) {
                // As we render the BG here we need to clip things (for layout
                // we would not)
                // FIXME: This cancels the OuterPadding addition done by
                // TableGetCellBgRect(), need to keep it while rendering
                // correctly while scrolling.
                const ImGuiTableColumn* column =
                    &table->Columns[cell_data->Column];
                ImRect cell_bg_rect =
                    TableGetCellBgRect(table, cell_data->Column);
                cell_bg_rect.ClipWith(table->BgClipRect);
                cell_bg_rect.Min.x =
                    ImMax(cell_bg_rect.Min.x,
                          column->ClipRect.Min
                              .x); // So that first column after frozen one gets
                                   // clipped when scrolling
                cell_bg_rect.Max.x = ImMin(cell_bg_rect.Max.x, column->MaxX);
                if (cell_bg_rect.Min.y < cell_bg_rect.Max.y)
                    window->DrawList->AddRectFilled(
                        cell_bg_rect.Min, cell_bg_rect.Max, cell_data->BgColor);
            }
        }
    ...
    // cursor and alter clipping rectangle) We need to do that in TableEndRow()
    // instead of TableBeginRow() so the list clipper can mark end of row and
    ...
    if (unfreeze_rows_request) {
        ...
        if (unfreeze_rows_actual) {
            ...
            IM_ASSERT(table->Bg2ClipRectForDrawCmd.Min.y <=
            ...
            // Update cliprect ahead of TableBeginCell() so clipper can access
            // to new ClipRect->Min.y
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n) {
    ...
    if (table->Flags & ImGuiTableFlags_NoClip) {
        // FIXME: if we end up drawing all borders/bg in EndTable, could remove
        // this and just assert that channel hasn't changed.
        table->DrawSplitter->SetCurrentChannel(window->DrawList,
                                               TABLE_DRAW_CHANNEL_NOCLIP);
...
float ImGui::TableGetMaxColumnWidth(const ImGuiTable* table, int column_n) {
    ...
    if (table->Flags & ImGuiTableFlags_ScrollX) {
        ...
        // naturally break. (we use DisplayOrder as within a set of multiple
        // frozen column reordering is possible)
        ...
        // If horizontal scrolling if disabled, we apply a final lossless
        // shrinking of columns in order to make sure they are all visible.
        // Because of this we also know that all of the columns will always fit
        // in table->WorkRect and therefore in table->InnerRect (because ScrollX
        // is off)
...
void ImGui::TableSetColumnWidth(int column_n, float width) {
    ...
    const float max_width =
        ImMax(min_width, TableGetMaxColumnWidth(table, column_n));
    ...
    // In this surprisingly not simple because of how we support mixing Fixed
    // and multiple Stretch columns.
    // - All fixed: easy.
    // - All stretch: easy.
    // - One or more fixed + one stretch: easy.
    // - One or more fixed + more than one stretch: tricky.
    // Qt when manual resize is enabled only supports a single _trailing_
    // stretch column, we support more cases here.
    ...
    // Fixed column. If active, ScrollX extent can be altered, but it doesn't
    // make much sense as the Stretch column will always be minimal size.
    // - F1 F2 W3  resize from W3|          --> ok: no-op (disabled by Resize
    // Rule 1)
    // - W1 W2 W3  resize from W1| or W2|   --> ok
    // - W1 W2 W3  resize from W3|          --> ok: no-op (disabled by Resize
    // Rule 1)
    // - W1 F2 F3  resize from F3|          --> ok: no-op (disabled by Resize
    // Rule 1)
    // - W1 F2     resize from F2|          --> ok: no-op (disabled by Resize
    // Rule 1)
    // - W1 W2 F3  resize from W1| or W2|   --> ok
    // - W1 F2 W3  resize from W1| or F2|   --> ok
    // - F1 W2 F3  resize from W2|          --> ok
    // - F1 W3 F2  resize from W3|          --> ok
    // - W1 F2 F3  resize from W1|          --> ok: equivalent to resizing |F2.
    // F3 will not move.
    ...
    // - W3 F1 F2  resize W3|               --> to not be stuck past F1|, both
    // F1 and F2 would need to stretch down, which would be lossy or ambiguous.
    // Seems hard to fix.
    ...
    // after a Stretch one, we can do an offsetting resize. This is the
    // preferred resize path
    ...
    // Resizing from right-side of a Stretch column before a Fixed column
    // forward sizing to left-side of fixed column. (old_a + old_b == new_a +
    // new_b) --> (new_a == old_a + old_b - new_b)
    float column_1_width = ImMax(column_1->WidthRequest -
    ...
    if ((column_0->Flags | column_1->Flags) &
...
void ImGui::TableSetColumnWidthAutoAll(ImGuiTable* table) {
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
        ...
        if (!column->IsEnabled &&
            !(column->Flags &
              ImGuiTableColumnFlags_WidthStretch)) // Cannot reset weight of
                                                   // hidden stretch column
            continue;
        column->CannotSkipItemsQueue = (1 << 0);
...
void ImGui::TableUpdateColumnsWeightFromWidth(ImGuiTable* table) {
    IM_ASSERT(table->LeftMostStretchedColumn != -1 &&
...
void ImGui::TableSetupDrawChannels(ImGuiTable* table) {
    ...
    table->DummyDrawChannel =
        (ImGuiTableDrawChannelIdx)((channels_for_dummy > 0) ? channels_total - 1
                                                            : -1);
    ...
    // host, to facilitate merge draw commands by default. All our cell
    // highlight are manually clipped with BgClipRect. When unfreezing it will
    // be made smaller to fit scrolling rect. (This technically isn't part of
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table) {
    ...
    const int size_for_masks_bitarrays_one =
        (int)ImBitArrayGetStorageSizeInBytes(max_draw_channels);
    ...
    for (int n = 0; n < IM_ARRAYSIZE(merge_groups); n++)
        merge_groups[n].ChannelsMask =
            (ImBitArrayPtr)(void*)(g.TempBuffer.Data +
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
        ...
        for (int merge_group_sub_n = 0;
             merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++) {
            const int channel_no = (merge_group_sub_n == 0)
                                       ? column->DrawChannelFrozen
                                       : column->DrawChannelUnfrozen;

            // Don't attempt to merge if there are multiple draw calls within
            // the column
            ...
            // Find out the width of this merge group and check if it will fit
            // in our column (note that we assume that rendering didn't stray on
            // the left direction. we should need a CursorMinPos to detect it)
            if (!(column->Flags & ImGuiTableColumnFlags_NoClip)) {
                float content_max_x;
                if (!has_freeze_v)
                    content_max_x =
                        ImMax(column->ContentMaxXUnfrozen,
                              column->ContentMaxXHeadersUsed); // No row freeze
                else if (merge_group_sub_n == 0)
                    content_max_x = ImMax(
                        column->ContentMaxXFrozen,
                        column->ContentMaxXHeadersUsed); // Row freeze: use
                                                         // width before freeze
                else
                    content_max_x =
    ...
    if (merge_group_mask != 0) {
        ...
        ImBitArraySetBitRange(remaining_mask, LEADING_DRAW_CHANNELS,
                              splitter->_Count);
        ImBitArrayClearBit(remaining_mask, table->Bg2DrawChannelUnfrozen);
        IM_ASSERT(has_freeze_v == false || table->Bg2DrawChannelUnfrozen !=
                                               TABLE_DRAW_CHANNEL_BG2_FROZEN);
        int remaining_count =
            splitter->_Count -
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups);
             merge_group_n++) {
            if (int merge_channels_count =
                ...
                // Extend outer-most clip limits to match those of host, so draw
                // calls can be merged even if outer-most columns have some
                // outer padding offsetting them from their parent ClipRect. The
                // principal cases this is dealing with are:
                // - On a same-window table (not scrolling = single group), all
                // fitting columns ClipRect -> will extend and match host
                // ClipRect -> will merge
                // - Columns can use padding and have left-most ClipRect.Min.x
                // and right-most ClipRect.Max.x != from host ClipRect -> will
                // extend and match host ClipRect -> will merge
                // FIXME-TABLE FIXME-WORKRECT: We are wasting a merge
                // opportunity on tables without scrolling if column doesn't fit
                // within host clip rect, solely because of the half-padding
                // difference between window->WorkRect and
                // window->InnerClipRect.
                if ((merge_group_n & 1) == 0 || !has_freeze_h)
                    merge_clip_rect.Min.x =
                        ImMin(merge_clip_rect.Min.x, host_rect.Min.x);
                if ((merge_group_n & 2) == 0 || !has_freeze_v)
                    merge_clip_rect.Min.y =
                        ImMin(merge_clip_rect.Min.y, host_rect.Min.y);
                if ((merge_group_n & 1) != 0)
                    merge_clip_rect.Max.x =
                        ImMax(merge_clip_rect.Max.x, host_rect.Max.x);
                if ((merge_group_n & 2) != 0 &&
                    (table->Flags & ImGuiTableFlags_NoHostExtendY) == 0)
                    merge_clip_rect.Max.y =
                        ImMax(merge_clip_rect.Max.y, host_rect.Max.y);
                // GetForegroundDrawList()->AddRect(merge_group->ClipRect.Min,
                // merge_group->ClipRect.Max, IM_COL32(255, 0, 0, 200), 0.0f,
                // 0, 1.0f); // [DEBUG]
                ...
                for (int n = 0;
                     n < splitter->_Count && merge_channels_count != 0; n++) {
                    ...
                    IM_ASSERT(channel->_CmdBuffer.Size == 1 &&
                              merge_clip_rect.Contains(
                                  ImRect(channel->_CmdBuffer[0].ClipRect)));
                    channel->_CmdBuffer[0].ClipRect = merge_clip_rect.ToVec4();
                    memcpy(dst_tmp++, channel, sizeof(ImDrawChannel));
                }
            }

            // Make sure Bg2DrawChannelUnfrozen appears in the middle of our
            // groups (whereas Bg0/Bg1 and Bg2 frozen are fixed to 0 and 1)
        ...
        // Append unmergeable channels that we didn't reorder at the end of the
        // list
...
static ImU32 TableGetColumnBorderCol(ImGuiTable* table, int order_n,
                                     int column_n) {
    ...
    if (is_resized || is_hovered)
        return ImGui::GetColorU32(is_resized ? ImGuiCol_SeparatorActive
...
void ImGui::TableDrawBorders(ImGuiTable* table) {
        ...
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
            ...
            if (column->NextEnabledColumn == -1 && !is_resizable)
                if ((table->Flags & ImGuiTableFlags_SizingMask_) !=
                    ...
                    continue;
            if (column->MaxX <=
                column->ClipRect.Min.x) // FIXME-TABLE FIXME-STYLE: Assume
                                        // BorderSize==1, this is problematic if
                                        // we want to increase the border size..
                continue;

            // Draw in outer window so right-most column won't be clipped
            // Always draw full height border when being resized/hovered, or on
            // the delimitation of frozen column scrolling.
            float draw_y2 =
    ...
    if (table->Flags & ImGuiTableFlags_BordersOuter) {
        ...
        // without adding an extra draw call (Without the offset, in
        // outer_window it would be rendered behind cells, because child windows
        // are above their parent. In inner_window, it won't reach out over
        // scrollbars. Another weird solution would be to display part of it in
        // inner window, and the part that's over scrollbars in the outer
        // window..) Either solution currently won't allow us to use a larger
        // border size: the border would clipped.
        const ImRect outer_border = table->OuterRect;
        const ImU32 outer_col = table->BorderColorStrong;
        if ((table->Flags & ImGuiTableFlags_BordersOuter) ==
            ImGuiTableFlags_BordersOuter) {
            inner_drawlist->AddRect(outer_border.Min,
                                    outer_border.Max + ImVec2(1, 1), outer_col,
                                    0.0f, 0, border_size);
...
static inline ImGuiSortDirection
TableGetColumnAvailSortDirection(ImGuiTableColumn* column, int n) {
    IM_ASSERT(n < column->SortDirectionsAvailCount);
    return (ImGuiSortDirection)((column->SortDirectionsAvailList >> (n << 1)) &
                                0x03);
}
...
ImGuiSortDirection
ImGui::TableGetColumnNextSortDirection(ImGuiTableColumn* column) {
...
void ImGui::TableSetColumnSortDirection(int column_n,
    ...
    if (append_to_sort_specs)
        for (int other_column_n = 0; other_column_n < table->ColumnsCount;
             other_column_n++)
            sort_order_max =
                ImMax(sort_order_max, table->Columns[other_column_n].SortOrder);
    ...
    if (sort_order_count == 0 && !(table->Flags & ImGuiTableFlags_SortTristate))
        for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
            ...
            if (column->IsEnabled &&
                ...
                column->SortOrder = 0;
                column->SortDirection =
                    (ImU8)TableGetColumnAvailSortDirection(column, 0);
                break;
            }
        }
...
void ImGui::TableSortSpecsBuild(ImGuiTable* table) {
    ...
    // Write output
    // May be able to move all SortSpecs data from table (48 bytes) to
    // ImGuiTableTempData if we decide to write it back on every BeginTable()
...
float ImGui::TableGetHeaderRowHeight() {
    ...
    // highlight but smaller one before the tallest wouldn't catch input for all
    // height. In your custom header row you may omit this all together and just
    // call TableNextRow() without a height...
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        if (IM_BITARRAY_TESTBIT(table->EnabledMaskByIndex, column_n))
            if ((table->Columns[column_n].Flags &
                 ImGuiTableColumnFlags_NoHeaderLabel) == 0)
                row_height =
                    ImMax(row_height,
...
// can create their own! For example: TableHeader() may be preceded by
...
void ImGui::TableHeadersRow() {
    ...
    // Layout if not already done (this is automatically done by TableNextRow,
    // we do it here solely to facilitate stepping in debugger as it is frequent
    // to step in TableUpdateLayout)
    if (!table->IsLayoutLocked)
        TableUpdateLayout(table);
    ...
    if (table->HostSkipItems) // Merely an optimization, you may skip in your
                              // own code.
        return;
    ...
    for (int column_n = 0; column_n < columns_count; column_n++) {
        ...
        // behave nicely with them) In your own code you may omit the
        // PushID/PopID all-together, provided you know they won't collide.
...
void ImGui::TableHeader(const char* label) {
    ...
    float label_height = ImMax(label_size.y, table->RowMinHeight -
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) &&
        ...
        if (column->SortOrder > 0) {
            ImFormatString(sort_order_suf, IM_ARRAYSIZE(sort_order_suf), "%d",
                           column->SortOrder + 1);
            w_sort_text =
                g.Style.ItemInnerSpacing.x + CalcTextSize(sort_order_suf).x;
        }
    }

    // We feed our unclipped width to the column without writing on
    // CursorMaxPos, so that column is still considered for merging.
    float max_pos_x = label_pos.x + label_size.x + w_sort_text + w_arrow;
    column->ContentMaxXHeadersUsed =
        ImMax(column->ContentMaxXHeadersUsed,
              sort_arrow ? cell_r.Max.x : ImMin(max_pos_x, cell_r.Max.x));
    ...
    ItemSize(
        ImVec2(0.0f, label_height)); // Don't declare unclipped width, it'll be
                                     // fed ContentMaxPosHeadersIdeal
    if (!ItemAdd(bb, id))
        return;

    // GetForegroundDrawList()->AddRect(cell_r.Min, cell_r.Max, IM_COL32(255, 0,
    ...
    bool hovered, held;
    bool pressed =
        ButtonBehavior(bb, id, &hovered, &held, ImGuiButtonFlags_AllowOverlap);
    if (held || hovered || highlight) {
        ...
        // Submit single cell bg color in the case we didn't submit a full
        // header row
    ...
    if (held && (table->Flags & ImGuiTableFlags_Reorderable) &&
        ...
        // While moving a column it will jump on the other side of the mouse, so
        // we also test for MouseDelta.x
        table->ReorderColumn = (ImGuiTableColumnIdx)column_n;
        table->InstanceInteracted = table->InstanceCurrent;

        // We don't reorder: through the frozen<>unfrozen line, or through a
        // column that is marked with ImGuiTableColumnFlags_NoReorder.
    ...
    const float ellipsis_max =
        ImMax(cell_r.Max.x - w_arrow - w_sort_text, label_pos.x);
    if ((table->Flags & ImGuiTableFlags_Sortable) &&
        !(column->Flags & ImGuiTableColumnFlags_NoSort)) {
        if (column->SortOrder != -1) {
            ...
            if (column->SortOrder > 0) {
                ...
                RenderText(ImVec2(x + g.Style.ItemInnerSpacing.x, y),
                           sort_order_suf);
                PopStyleColor();
                x += w_sort_text;
            }
            RenderArrow(window->DrawList, ImVec2(x, y),
        ...
        if (pressed && table->ReorderColumn != column_n) {
            ImGuiSortDirection sort_direction =
                TableGetColumnNextSortDirection(column);
    ...
    // window->DrawList->AddCircleFilled(ImVec2(ellipsis_max, label_pos.y), 40,
    // IM_COL32_WHITE);
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        if (IM_BITARRAY_TESTBIT(table->EnabledMaskByDisplayOrder, order_n)) {
            ...
            if ((column->Flags & ImGuiTableColumnFlags_AngledHeader) ==
                0) // Note: can't rely on ImGuiTableColumnFlags_IsVisible test
                   // here.
                continue;
            ImGuiTableHeaderData request = {
                (ImGuiTableColumnIdx)column_n, col_text, col_header_bg,
                (column_n == highlight_column_n) ? GetColorU32(ImGuiCol_Header)
                                                 : 0};
...
void ImGui::TableAngledHeadersRowEx(ImGuiID row_id, float angle,
    ...
    // Angle argument expressed in (-IM_PI/2 .. +IM_PI/2) as it is easier to
    ...
    const float row_height = ImTrunc(
        ImFabs(ImRotate(ImVec2(max_label_width,
    ...
    const ImVec2 header_angled_vector =
        unit_right *
        (row_height / -sin_a); // vector from bottom-left to top-left, and from
                               // bottom-right to top-right

    // Declare row, override and draw our own background
    TableNextRow(ImGuiTableRowFlags_Headers, row_height);
    TableNextColumn();
    const ImRect row_r(table->WorkRect.Min.x, table->BgClipRect.Min.y,
                       table->WorkRect.Max.x, table->RowPosY2);
    table->DrawSplitter->SetCurrentChannel(draw_list, TABLE_DRAW_CHANNEL_BG0);
    float clip_rect_min_x = table->BgClipRect.Min.x;
    if (table->FreezeColumnsCount > 0)
        clip_rect_min_x =
            ImMax(clip_rect_min_x,
    ...
    draw_list->AddRectFilled(
        ImVec2(table->BgClipRect.Min.x, row_r.Min.y),
        ImVec2(table->BgClipRect.Max.x, row_r.Max.y),
        GetColorU32(ImGuiCol_TableHeaderBg,
                    0.25f)); // FIXME-STYLE: Change row background with an
                             // arbitrary color.
    PushClipRect(ImVec2(clip_rect_min_x, table->BgClipRect.Min.y),
    ...
    const float ascent_scaled =
        g.Font->Ascent *
        g.FontScale; // FIXME: Standardize those scaling factors better
    const float line_off_for_ascent_x =
        (ImMax((g.FontSize - ascent_scaled) * 0.5f, 0.0f) / -sin_a) *
        (flip_label ? -1.0f : 1.0f);
    const ImVec2 padding =
        g.Style.CellPadding; // We will always use swapped component
    ...
    for (int pass = 0; pass < 2; pass++)
        for (int order_n = 0; order_n < data_count; order_n++) {
            ...
            if (pass == 0) {
                ...
                // Draw label
                // - First draw at an offset where RenderTextXXX() function
                // won't meddle with applying current ClipRect, then transform
                // to final offset.
                // - Handle multiple lines manually, as we want each lines to
                // follow on the horizontal border, rather than see a whole
                // block rotated.
                const char* label_name = TableGetColumnName(table, column_n);
                const char* label_name_end = FindRenderedTextEnd(label_name);
                const float line_off_step_x = (g.FontSize / -sin_a);
                const int label_lines =
                    ImTextCountLines(label_name, label_name_end);

                // Left<>Right alignment
                float line_off_curr_x =
                    flip_label ? (label_lines - 1) * line_off_step_x : 0.0f;
                float line_off_for_align_x =
                    ImMax((((column->MaxX - column->MinX) - padding.x * 2.0f) -
                ...
                column->ContentMaxXHeadersUsed =
                    column->ContentMaxXHeadersIdeal =
                        column->WorkMinX +
                        ImCeil(label_lines * line_off_step_x -
                ...
                while (label_name < label_name_end) {
                    ...
                    // FIXME: Individual line clipping for right-most column is
                    // broken for negative angles.
                    ImVec2 label_size =
                        CalcTextSize(label_name, label_name_eol);
                    float clip_width =
                        max_label_width -
                        padding.y; // Using padding.y*2.0f would be symmetrical
                                   // but hide more text.
                    float clip_height = ImMin(
                        label_size.y, column->ClipRect.Max.x -
                                          column->WorkMinX - line_off_curr_x);
                    ImRect clip_r(window->ClipRect.Min,
                                  window->ClipRect.Min +
                                      ImVec2(clip_width, clip_height));
                    int vtx_idx_begin = draw_list->_VtxCurrentIdx;
                    PushStyleColor(ImGuiCol_Text, request->TextColor);
                    RenderTextEllipsis(draw_list, clip_r.Min, clip_r.Max,
                                       clip_r.Max.x, clip_r.Max.x, label_name,
                    ...
                    const float available_space =
                        ImMax(clip_width - label_size.x +
                                  ImAbs(padding.x * cos_a) * 2.0f -
                                  ImAbs(padding.y * sin_a) * 2.0f,
                              0.0f);
                    const float vertical_offset =
                        available_space * align.y * (flip_label ? -1.0f : 1.0f);

                    // Rotate and offset label
                    ImVec2 pivot_in =
                        ImVec2(window->ClipRect.Min.x - vertical_offset,
                               window->ClipRect.Min.y + label_size.y);
                    ImVec2 pivot_out = ImVec2(column->WorkMinX, row_r.Max.y);
                    line_off_curr_x +=
                        flip_label ? -line_off_step_x : line_off_step_x;
                    pivot_out += unit_right * padding.y;
                    if (flip_label)
                        pivot_out += unit_right *
                                     (clip_width -
                                      ImMax(0.0f, clip_width - label_size.x));
                    pivot_out.x += flip_label
                                       ? line_off_curr_x + line_off_step_x
                                       : line_off_curr_x;
                    ShadeVertsTransformPos(draw_list, vtx_idx_begin,
                                           vtx_idx_end, pivot_in, label_cos_a,
                                           label_sin_a,
                                           pivot_out); // Rotate and offset
                    // if (g.IO.KeyShift) { ImDrawList* fg_dl =
                    // GetForegroundDrawList(); vtx_idx_begin =
                    // fg_dl->_VtxCurrentIdx; fg_dl->AddRect(clip_r.Min,
                    // clip_r.Max, IM_COL32(0, 255, 0, 255), 0.0f, 0, 1.0f);
                    // ShadeVertsTransformPos(fg_dl, vtx_idx_begin,
...
void ImGui::TableDrawDefaultContextMenu(
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Resizable) {
        ...
        if (table->ColumnsEnabledFixedCount == table->ColumnsEnabledCount &&
            ...
            size_all_desc = LocalizeGetMsg(
                ImGuiLocKey_TableSizeAllFit); // "###SizeAll" All fixed
        else
            size_all_desc = LocalizeGetMsg(
                ImGuiLocKey_TableSizeAllDefault); // "###SizeAll" All stretch or
                                                  // mixed
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Hideable) {
        ...
        for (int other_column_n = 0; other_column_n < table->ColumnsCount;
            ...
            bool menu_item_active =
                (other_column->Flags & ImGuiTableColumnFlags_NoHide) ? false
                                                                     : true;
            if (other_column->IsUserEnabled && table->ColumnsEnabledCount <= 1)
                menu_item_active = false;
            if (MenuItem(name, NULL, other_column->IsUserEnabled,
                         menu_item_active))
...
void ImGui::TableResetSettings(ImGuiTable* table) {
    ...
    table->SettingsLoadedFlags =
        ImGuiTableFlags_None; // Mark as nothing loaded so our initialized data
...
void ImGui::TableSaveSettings(ImGuiTable* table) {
    ...
    for (int n = 0; n < table->ColumnsCount; n++, column++, column_settings++) {
        ...
        column_settings->IsStretch =
            (column->Flags & ImGuiTableColumnFlags_WidthStretch) ? 1 : 0;
        if ((column->Flags & ImGuiTableColumnFlags_WidthStretch) == 0)
            save_ref_scale = true;

        // We skip saving some data in the .ini file when they are unnecessary
        // to restore our state. Note that fixed width where initial width was
        // derived from auto-fit will always be saved as
...
static void* TableSettingsHandler_ReadOpen(ImGuiContext*, ImGuiSettingsHandler*,
                                           const char* name) {
    ImGuiID id = 0;
    int columns_count = 0;
...
static void TableSettingsHandler_ReadLine(ImGuiContext*, ImGuiSettingsHandler*,
    ...
    if (sscanf(line, "Column %d%n", &column_n, &r) == 1) {
        ...
        if (sscanf(line, "Sort=%d%c%n", &n, &c, &r) == 2) {
            ...
            column->SortDirection = (c == '^') ? ImGuiSortDirection_Descending
...
static void TableSettingsHandler_WriteAll(ImGuiContext* ctx,
    ...
    for (ImGuiTableSettings* settings = g.SettingsTables.begin();
        ...
        // TableSaveSettings() may clear some of those flags when we establish
        // that the data can be stripped (e.g. Order was unchanged)
        ...
        for (int column_n = 0; column_n < settings->ColumnsCount;
             column_n++, column++) {
            // "Column 0  UserID=0x42AD2D21 Width=100 Visible=1 Order=0 Sort=0v"
            bool save_column = column->UserID != 0 || save_size ||
                               save_visible || save_order ||
...
static const char*
DebugNodeTableGetSizingPolicyDesc(ImGuiTableFlags sizing_policy) {
...
void ImGui::DebugNodeTable(ImGuiTable* table) {
    ...
    const bool is_active =
        (table->LastFrameActive >=
         g.FrameCount - 2); // Note that fully clipped early out scrolling
                            // tables will appear as inactive here.
    if (!is_active) {
        PushStyleColor(ImGuiCol_Text, GetStyleColorVec4(ImGuiCol_TextDisabled));
    }
    bool open =
        TreeNode(table, "Table 0x%08X (%d columns, in '%s')%s", table->ID,
    ...
    BulletText("ColumnsGivenWidth: %.1f, ColumnsAutoFitWidth: %.1f, "
               "InnerWidth: %.1f%s",
               table->ColumnsGivenWidth, table->ColumnsAutoFitWidth,
               table->InnerWidth, table->InnerWidth == 0.0f ? " (auto)" : "");
    BulletText(
    ...
    for (int n = 0; n < table->ColumnsCount; n++) {
        ...
        ImFormatString(
            ...
            column->IsEnabled, column->IsVisibleX, column->IsVisibleY,
        ...
        Selectable(buf);
        if (IsItemHovered()) {
            ImRect r(column->MinX, table->OuterRect.Min.y, column->MaxX,
                     table->OuterRect.Max.y);
...
static float GetDraggedColumnOffset(ImGuiOldColumns* columns,
    ...
    // that dragging a column to the right edge of an auto-resizing window
    // creates a feedback loop because we store normalized positions. So while
    // dragging we enforce absolute positioning.
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    float x = g.IO.MousePos.x - g.ActiveIdClickOffset.x +
              ImTrunc(COLUMNS_HIT_RECT_HALF_THICKNESS * g.CurrentDpiScale) -
              window->Pos.x;
...
void ImGui::SetColumnOffset(int column_index, float offset) {
    ...
    if (!(columns->Flags & ImGuiOldColumnFlags_NoForceWithinWindow))
        offset = ImMin(offset,
                       columns->OffMaxX - g.Style.ColumnsMinSpacing *
    ...
    if (preserve_width)
        SetColumnOffset(column_index + 1,
                        offset + ImMax(g.Style.ColumnsMinSpacing, width));
}
...
ImGuiID ImGui::GetColumnsID(const char* str_id, int columns_count) {
    ...
    // Differentiate column ID with an arbitrary prefix for cases where users
...
void ImGui::BeginColumns(const char* str_id, int columns_count,
    ...
    const float half_clip_extend_x = ImTrunc(
...
void ImGui::EndColumns() {
    ...
    if (!(flags & ImGuiOldColumnFlags_GrowParentContentsSize))
        window->DC.CursorMaxPos.x =
            columns->HostCursorMaxPosX; // Restore cursor max pos, as columns
                                        // don't grow parent
    ...
    if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems) {
        ...
        for (int n = 1; n < columns->Count; n++) {
            ...
            window->DrawList->AddLine(ImVec2(xi, y1 + 1.0f), ImVec2(xi, y2),
                                      col);
        }

        // Apply dragging after drawing the column lines, so our rendered lines
        // are in sync with how items were displayed during the frame.
