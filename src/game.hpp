...
class Game{
    ...
    public:
        ...
        raylib::Rectangle render_rect(){
            raylib::Rectangle out(-raylib::Vector2(m_render_view.GetTexture().GetSize()));
            out.SetWidth(-out.GetWidth());
            return out;
        }

        Game(int window_width, int window_height, std::string window_name,
            int render_width, int render_height, int fps_max = 200) : m_window(window_width,window_height,window_name),
