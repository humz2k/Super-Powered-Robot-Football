...
class Selectable : public Component {
  ...
  public:
    Selectable(bool highlight = true, bool clickable = false)
        : m_id(next_id), m_highlight(highlight), m_clickable(clickable) {
        // TraceLog(LOG_INFO,"assigned id = %d",m_id);
        next_id++;
    }

    BoundingBox bounding_box() {
        return m_bbox.transform(this->entity()->global_transform())
            .axis_align();
    }
    ...
    void init() {
        if (this->entity()->has_component<Model>()) {
            auto model =
                this->entity()->get_component<Model>()->render_model()->model();
    ...
    void update() {
        if (!m_clickable)
            return;
        if (!(IsMouseButtonPressed(0) && IsKeyDown(KEY_LEFT_SHIFT)))
            return;
    ...
    void select() {
        if (Selectable::currently_selected) {
            Selectable::currently_selected->get_component<Selectable>()
                ->unselect();
        }
        m_selected = true;
...
class IMGuiManager : public Component {
  private:
    void parse_entity_tree(Entity* entity) {
        std::string entity_name =
            "(" + std::to_string(entity->id()) + ") " + entity->name();
  ...
  public:
    void before_draw2D() { rlImGuiBegin(); }

    void after_draw2D() { rlImGuiEnd(); }
